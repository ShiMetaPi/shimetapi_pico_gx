...
)
{
	...
	/*
	 * There are 3 IN eps(exclude ctrl in) in chip, which map to TXFIFO1~3,
	 * TXFIFO1 size = TXFIFO2 size > TXFIFO3 size, interrupt ep need less
	 *  bandwidth compared to other ep, so let interrupt ep match TXFIFO3.
	 */
