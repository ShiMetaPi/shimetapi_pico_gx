...
/*****************************************************************************/
struct nand_sync_timing lotus_fmc100_nand_sync_timing[] = {
...
/*****************************************************************************/
static void lotus_fmc100_set_timing_3_3v(struct nand_sync_timing *nand_sync_timing)
{
...
static void lotus_fmc100_set_timing_1_8v(struct nand_sync_timing *nand_sync_timing)
{
	unsigned int i, regval;
	u8 syncmode = nand_sync_timing->syncmode;
...
unsigned int get_nand_io_voltage(void)
{
	...
	virtaddr = ioremap(REG_BASE_SCTL + REG_SC_GEN29, PAGE_SIZE);
	if (!virtaddr) {
...
void lotus_fmc100_set_sync_timing(struct nand_sync_timing *nand_sync_timing)
{
	...
	virtaddr_ioshare = ioremap(REG_BASE_IOSHARE, PAGE_SIZE);
	if (!virtaddr_ioshare) {
	...
	if (io_voltage == NAND_IO_VOL_3_3V)
		return lotus_fmc100_set_timing_3_3v(nand_sync_timing);
	else
		return lotus_fmc100_set_timing_1_8v(nand_sync_timing);
}
