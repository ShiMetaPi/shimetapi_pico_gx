...
#if defined(CONFIG_ARCH_XMFALCON) || defined(CONFIG_ARCH_XMORCA)
static void uvcg_framebased_format_set_indices(struct config_group *fmt);
#endif
...
#if defined(CONFIG_ARCH_XMFALCON) || defined(CONFIG_ARCH_XMORCA)
	if (target_fmt->type == UVCG_FRAMEBASED) {
		uvcg_framebased_format_set_indices(to_config_group(target));
	} else {
...
static struct uvcg_framebased_frame *to_uvcg_framebased_frame(struct config_item *item)
{
	return container_of(item, struct uvcg_framebased_frame, item);
}

#define UVCG_FRAMEBASED_FRAME_ATTR(cname, aname, bits) \
static ssize_t uvcg_framebased_frame_##cname##_show(struct config_item *item, char *page)\
{									\
	struct uvcg_framebased_frame *f = to_uvcg_framebased_frame(item);			\
	struct f_uvc_opts *opts;					\
	struct config_item *opts_item;					\
	struct mutex *su_mutex = &f->item.ci_group->cg_subsys->su_mutex;\
	int result;							\
	...
	return result;							\
									...
									\
static ssize_t  uvcg_framebased_frame_##cname##_store(struct config_item *item,	\
					   const char *page, size_t len)\
{									\
	struct uvcg_framebased_frame *f = to_uvcg_framebased_frame(item);			\
	struct f_uvc_opts *opts;					\
	...
	return ret;							\
									...
									\
UVC_ATTR(uvcg_framebased_frame_, cname, aname);

static ssize_t uvcg_framebased_frame_b_frame_index_show(struct config_item *item,
					     char *page)
{
	struct uvcg_framebased_frame *f = to_uvcg_framebased_frame(item);
	struct uvcg_format *fmt;
...
UVC_ATTR_RO(uvcg_framebased_frame_, b_frame_index, bFrameIndex);

UVCG_FRAMEBASED_FRAME_ATTR(bm_capabilities, bmCapabilities, 8);
UVCG_FRAMEBASED_FRAME_ATTR(w_width, wWidth, 16);
UVCG_FRAMEBASED_FRAME_ATTR(w_height, wHeight, 16);
UVCG_FRAMEBASED_FRAME_ATTR(dw_min_bit_rate, dwMinBitRate, 32);
UVCG_FRAMEBASED_FRAME_ATTR(dw_max_bit_rate, dwMaxBitRate, 32);
UVCG_FRAMEBASED_FRAME_ATTR(dw_default_frame_interval, dwDefaultFrameInterval, 32);
UVCG_FRAMEBASED_FRAME_ATTR(dw_bytes_per_line, dwBytesPerLine, 32);

#undef UVCG_FRAMEBASED_FRAME_ATTR

static ssize_t uvcg_framebased_frame_dw_frame_interval_show(struct config_item *item,
						 char *page)
{
	struct uvcg_framebased_frame *frm = to_uvcg_framebased_frame(item);
	struct f_uvc_opts *opts;
	struct config_item *opts_item;
	struct mutex *su_mutex = &frm->item.ci_group->cg_subsys->su_mutex;
	int result, i;
...
static ssize_t uvcg_framebased_frame_dw_frame_interval_store(struct config_item *item,
						  const char *page, size_t len)
{
	struct uvcg_framebased_frame *ch = to_uvcg_framebased_frame(item);
	struct f_uvc_opts *opts;
	struct config_item *opts_item;
	struct uvcg_format *fmt;
	struct mutex *su_mutex = &ch->item.ci_group->cg_subsys->su_mutex;
...
UVC_ATTR(uvcg_framebased_frame_, dw_frame_interval, dwFrameInterval);

static struct configfs_attribute *uvcg_framebased_frame_attrs[] = {
	&uvcg_framebased_frame_attr_b_frame_index,
	&uvcg_framebased_frame_attr_bm_capabilities,
	&uvcg_framebased_frame_attr_w_width,
	&uvcg_framebased_frame_attr_w_height,
	&uvcg_framebased_frame_attr_dw_min_bit_rate,
	&uvcg_framebased_frame_attr_dw_max_bit_rate,
	&uvcg_framebased_frame_attr_dw_default_frame_interval,
	&uvcg_framebased_frame_attr_dw_frame_interval,
	&uvcg_framebased_frame_attr_dw_bytes_per_line,
	NULL,
};

static const struct config_item_type uvcg_framebased_frame_type = {
	.ct_item_ops	= &uvcg_config_item_ops,
	.ct_attrs	= uvcg_framebased_frame_attrs,
	.ct_owner	= THIS_MODULE,
};

static struct config_item *uvcg_framebased_frame_make(struct config_group *group,
					   const char *name)
{
	struct uvcg_framebased_frame *h;
	struct uvcg_format *fmt;
	struct f_uvc_opts *opts;
	struct config_item *opts_item;
	...
	config_item_init_type_name(&h->item, name, &uvcg_framebased_frame_type);

	return &h->item;
}

static void uvcg_framebased_frame_drop(struct config_group *group, struct config_item *item)
{
	struct uvcg_format *fmt;
	struct f_uvc_opts *opts;
	struct config_item *opts_item;
...
static void uvcg_framebased_format_set_indices(struct config_group *fmt)
{
	...
	list_for_each_entry(ci, &fmt->cg_children, ci_entry) {
		struct uvcg_framebased_frame *frm;

		if (ci->ci_type != &uvcg_framebased_frame_type)
			continue;

		frm = to_uvcg_framebased_frame(ci);
		frm->frame.b_frame_index = i++;
	}
}
#endif /* CONFIG_ARCH_XMFALCON || CONFIG_ARCH_XMORCA */
...
static struct configfs_group_operations uvcg_framebased_group_ops = {
	.make_item		= uvcg_framebased_frame_make,
	.drop_item		= uvcg_framebased_frame_drop,
};
...
static ssize_t uvcg_framebased_##cname##_show(			\
	struct config_item *item, char *page)				\
{									\
...
{
	struct uvcg_framebased *unc = to_uvcg_framebased(item);
	return uvcg_format_bma_controls_show(&unc->fmt, page);
}
			    ...
			    enum uvcg_strm_type type)
{
	...
	**array = *dest;
	...
	case UVCG_FORMAT: {
		...
		if (fmt->type == UVCG_UNCOMPRESSED) {
			...
			*dest += sizeof(m->desc);
			...
			fb->desc.bFormatIndex = n + 1;
			fb->desc.bNumFrameDescriptors = fmt->num_frames;
			memcpy(*dest, &fb->desc, sizeof(fb->desc));
			*dest += sizeof(fb->desc);
#endif
		} else {
			return -EINVAL;
	...
	case UVCG_FRAME: {
		struct uvcg_frame *frm = priv1;
		struct uvcg_framebased_frame *frm_fb = priv1;
		...
		*dest += sz;
		...
		if (frm->fmt_type == UVCG_FRAMEBASED) {
			sz = frm_fb->frame.b_frame_interval_type *
				sizeof(*frm_fb->dw_frame_interval);
		} else {
		...
		*dest += sz;
		if (frm->fmt_type == UVCG_UNCOMPRESSED)
			...
			h->bLength = UVC_DT_FRAME_BASED_FRAME_SIZE(
				frm_fb->frame.b_frame_interval_type);
	}
#else
	case UVCG_FRAME: {
		struct uvcg_frame *frm = priv1;
