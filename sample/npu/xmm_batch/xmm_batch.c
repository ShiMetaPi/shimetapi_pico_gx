...
int main()
{
    ...
    xmedia_cl_s32 i = 0, j, size = 0, err, ret;
    xmedia_cl_s32 k = 0;
    ...
    srand(time(NULL));
    xmedia_cl_s32 batch = rand() % MAX_BATCH_NUM + 1;
    //公共模块初始化
    ...
    for (i = 0; i < input.num; i++) {
        ret = xmedia_cl_graph_set_dynamic_batch_size(graph, &input, i, batch);
        if (ret != XMEDIA_CL_SUCCESS) {
            printf("xmedia_cl_graph_set_dynamic_batch_size, errno %d\n", ret);
            goto ERROR;
        }
    }
    //计算输入的大小并申请内存
    ...
    ret = XMEDIA_API_SYS_MmzAlloc_Cached(&u64SrcPhyAddr[2],
                                         ...
                                         "npu_input", NULL, inputsize * batch);
    if (XMEDIA_CL_SUCCESS != ret) {
        goto ERROR;
    }
    //计算输出的大小并申请内存
    ...
    ret = XMEDIA_API_SYS_MmzAlloc_Cached(&u64SrcPhyAddr[3],
                                         ...
                                         "npu_output", NULL, outputsize * batch);
    if (XMEDIA_CL_SUCCESS != ret) {
        goto ERROR;
    }
    ...
    for (i = 0; i < input.num; i++) {
        ...
        for (j = 0; j < batch; j++) {
            memset(input.tensor[i].addr + j * ALIGN_FUNC(size, ALIGN_BYTE), 0,
                   size); //清0防止运行拷贝模式对非拷贝模式的影响
            sprintf(name, "data/input_data%d_%d.bin", j, i);
            ret = copy_file_to_buff(
                name, input.tensor[i].addr + j * ALIGN_FUNC(size, ALIGN_BYTE),
                0, size);
            if (ret != XMEDIA_CL_SUCCESS) {
                printf("read input_data%d_%d.bin failed!\n", j, i);
                goto ERROR;
            }
        }
    ...
    for (i = 0; i < output.num; i++) {
        ...
        buff[i] = malloc(ALIGN_FUNC(output_size[i], ALIGN_BYTE) * batch);
        if (buff[i] == NULL) {
            printf("malloc err\n");
            ret = XMEDIA_CL_OUT_OF_HOST_MEMORY;
            goto ERROR;
        ...
        for (j = 0; j < batch; j++) {
            sprintf(name, "data/output_data%d_%d.bin", j, i);
            ret = copy_file_to_buff(name, buff[i] + j * output_size[i], 0,
                                    output_size[i]);
            if (ret != XMEDIA_CL_SUCCESS) {
                printf("read output_data%d_%d.bin failed!\n", j, i);
                goto ERROR;
            }
            memset(output.tensor[i].addr + j * outputsize, 0, output_size[i]);
        }
    ...
    for (j = 0; j < 10; j++) {
        ...
        for (i = 0; i < output.num; i++) {
            for (k = 0; k < batch; k++) {
                printf("#################################\n");
                array_cmp_fixed(output.tensor[i].addr + k * outputsize,
                                buff[i] + k * output_size[i],
