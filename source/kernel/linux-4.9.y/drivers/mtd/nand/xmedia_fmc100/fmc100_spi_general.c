...
static int spi_general_disable_protect(struct fmc_spi *spi)
{
	unsigned int reg;
...
static int spi_general_disable_ecc(struct fmc_spi *spi)
{
	unsigned int reg;
	reg = spi_nand_feature_op(spi, GET_OP, FEATURE_ADDR, 0);
...
static int spi_fm_disable_protect(struct fmc_spi *spi)
{
	unsigned int reg;
...
static int spi_fm_disable_ecc(struct fmc_spi *spi)
{
	unsigned int reg;
	reg = spi_nand_feature_op(spi, GET_OP, FM_ECCEN_ADDR, 0);
	FMC_PR(BT_DBG, "\t||-Get feature status[%#x]: %#x\n", FM_ECCEN_ADDR,
	       reg);
	if (reg & FEATURE_ECC_ENABLE) {
		reg &= ~FEATURE_ECC_ENABLE;
		spi_nand_feature_op(spi, SET_OP, FM_ECCEN_ADDR, reg);
		FMC_PR(BT_DBG, "\t||-Set [%#x]FT: %#x\n", FM_ECCEN_ADDR, reg);
