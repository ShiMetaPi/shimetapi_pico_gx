                           ...
                           struct mmb_info *pmi)
{
    ...
    if (mmb == NULL) {
        ...
        error_mmz("mmz_mmb_alloc(%s, %llu, 0x%llx, %lu, %s) failed!\n",
              pmi->mmb_name, pmi->size, pmi->align,
              pmi->gfp, pmi->mmz_name);
#else
        error_mmz("mmz_mmb_alloc(%s, %lu, 0x%lx, %lu, %s) failed!\n",
              pmi->mmb_name, pmi->size, pmi->align,
              pmi->gfp, pmi->mmz_name);
#endif
        return -ENOMEM;
                                ...
                                int cached)
{
    ...
    struct mmb_info *p = NULL;
    mmz_mmb_t *mmb = NULL;
    unsigned long addr, len, prot, flags, pgoff, addr_offset;

    if ((p = get_mmbinfo_safe(pmi->phys_addr, pmu)) == NULL) {
        if ((mmb = mmz_mmb_getby_phys_2(pmi->phys_addr, &addr_offset)) == NULL) {
            error_mmz("mmb(0x%08lX) not found in kernel!\n", pmi->phys_addr);
            return -EPERM;
        } else {

            if (mmz_mmb_get(mmb) < 0) {
                error_mmz("get mmb failed!\n");
                return -EPERM;
            }

            if (pmi->size + addr_offset > mmb->length) {
                error_mmz("mmb<0x%08lX> map size(0x%lx) bigger than mmz size(0x%lx)\n",
                      pmi->phys_addr, pmi->size, mmb->length);
                mmz_mmb_put(mmb);
                return -EINVAL;
            }
            ...
            memset(p, 0, sizeof(*p));
            p->phys_addr = mmb->phys_addr;
            p->mmb = mmb;
            p->prot = PROT_READ;
            p->flags = MAP_SHARED;
            memcpy(p->mmb_name, mmb->name, MMZ_MMB_NAME_LEN);
            memcpy(p->mmz_name, mmb->zone->name, MMZ_MMZ_NAME_LEN);
            p->size = mmb->length;
            osal_list_add_tail(&p->list, &pmu->list);
            p->kernel_mmb = 1;
        }
       /*if map size is bigger than mmz size, may stampede memory*/
    } else if (pmi->size > p->size) {
        /*if map size is bigger than mmz size, may stampede memory*/
        error_mmz("mmb<%s> map size(0x%lx) bigger than mmz size(0x%lx)\n",
              p->mmb->name, pmi->size, p->size);
        return -EINVAL;
    }
    ...
    if (p->mapped && (p->map_ref > 0)) {
        /*用户空间使用内核空间的mmz，在这个错误返回中不需要释放内存.
         *因为第一次还没有map到用户空间，mapped参数为0，后面已经有使用者，不在这里释放 */
                                ...
                                unsigned int iocmd, struct mmb_info *pmi)
{
    ...
    phy_addr = usr_virt_to_phys(virt);
    if (!(phy_addr)) {
        error_mmz("usr virt to phys failed\n");
        return -ENOMEM;
    }

    if ((p = get_mmbinfo_safe(phy_addr, pmu)) == NULL) {
        error_mmz("get mmb info from phy addr failed\n");
        return -EPERM;
    }
    ...
    if (p->kernel_mmb && (p->map_ref == 0) && (p->mmb_ref == 0)) {
        osal_list_del(&p->list);
        mmz_mmb_put(p->mmb);
        kfree(p);
    } else if (p->delayed_free && (p->map_ref == 0) && (p->mmb_ref == 0)) {
        _usrdev_mmb_free(p);
    }
...
static long mmz_userdev_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
    ...
    if (pmu->tgid != current->tgid) {
        error_mmz("err!! not current process fd, please open after close\n");
        return -EPERM;
    }
...
#else
    if (memblock_is_map_memory(__pfn_to_phys(vma->vm_pgoff))) {
#endif
...
static int mmz_userdev_release(struct inode *inode, struct file *file)
{
    ...
    list_for_each_entry_safe(p, n, &pmu->list, list) {
        ...
        /*not alloc in this application, not free in this application*/
        if (p->kernel_mmb) {
            osal_list_del(&p->list);
            mmz_mmb_put(p->mmb);
            kfree(p);
