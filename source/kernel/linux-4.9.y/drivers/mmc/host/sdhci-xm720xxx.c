...
#define XMEDIA_MMC_AUTOSUSPEND_DELAY_MS 50
...
static int sdhci_bsp_pltfm_init(struct platform_device *pdev, struct sdhci_host *host)
{
	...
	/* For eMMC is always present. */
	if (bsp_priv->devid == 0) {
		regmap_read(bsp_priv->iocfg_regmap, REG_CTRL_EMMC_CLK, &reg);
...
static void bsp_get_phase(struct sdhci_host *host)
{
	...
	if (devid == 0) {
		...
		if (host->mmc->ios.timing == MMC_TIMING_MMC_HS400) {
			bsp_priv->drv_phase = 9/fix_num;   /* 9 for 101.25 degree */
			bsp_priv->sampl_phase = bsp_priv->tuning_phase/fix_num;
		} else if (host->mmc->ios.timing == MMC_TIMING_MMC_HS200) {
			bsp_priv->drv_phase = 23/fix_num;  /* 23 for 258.75 degree */
			bsp_priv->sampl_phase = bsp_priv->tuning_phase/fix_num;
		} else if (host->mmc->ios.timing == MMC_TIMING_MMC_HS) {
			bsp_priv->drv_phase = 16/fix_num;  /* 16 for 180 degree */
			bsp_priv->sampl_phase = 4/fix_num; /* 4 for 45 degree */
		} else if (host->mmc->ios.timing == MMC_TIMING_SD_HS) {
			bsp_priv->drv_phase = 20/fix_num;  /* 20 for 225 degree */
			bsp_priv->sampl_phase = 4/fix_num; /* 4 for 45 degree */
			...
			bsp_priv->drv_phase = 8/fix_num;   /* 8 for 90 degree */
			bsp_priv->sampl_phase = bsp_priv->tuning_phase/fix_num;
		} else {
			bsp_priv->drv_phase = 16/fix_num;  /* 16 for 180 degree */
			bsp_priv->sampl_phase = 0; /* 0 for 0 degree */
