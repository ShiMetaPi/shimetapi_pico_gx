...
#define RTC_FREQ_L		0x52
    ...
    defined(CONFIG_ARCH_XM72010200) || defined(CONFIG_ARCH_XM72010300)
...
static int xmedia_spi_rtc_write(void *spi_reg, unsigned char reg,
			       unsigned char val)
{
	return xmedia_spi_write(spi_reg, reg, val);
}
...
static int xmedia_spi_rtc_read(void *spi_reg, unsigned char reg,
			      unsigned char *val)
{
	return xmedia_spi_read(spi_reg, reg, val);
}
...
static int xmedia_rtc_read_time(struct device *dev, struct rtc_time *time)
{
	...
	ret = (unsigned int)xmedia_spi_rtc_read(rtc->regs, RTC_INT_RAW, &raw_value);
	if (ret) {
		dev_err(dev, "IO err.\n");
		return ret;
	}
	...
	if (raw_value & LV_INT_MASK) {
		...
		(unsigned int)xmedia_spi_write(rtc->regs, RTC_INT_CLR, 1);
		//return -EINVAL;
	}

	ret |= (unsigned int)xmedia_spi_rtc_read(rtc->regs, RTC_LORD, &raw_value);
	if (raw_value & REG_LOCK_BYPASS)
		ret |= (unsigned int)xmedia_spi_rtc_write(rtc->regs, RTC_LORD,
	...
	do {
		ret |= (unsigned int)xmedia_spi_rtc_read(rtc->regs, RTC_LORD, &raw_value);
		mdelay(RTC_SLEEP_TIME_MS);
	} while ((ret || (raw_value & REG_LOCK_STAT)) && (--cnt));

	if (!ret && (raw_value & REG_LOCK_STAT))
		return -EBUSY;

	ret |= (unsigned int)xmedia_spi_rtc_read(rtc->regs, RTC_S_COUNT, &second);
	ret |= (unsigned int)xmedia_spi_rtc_read(rtc->regs, RTC_M_COUNT, &minute);
	ret |= (unsigned int)xmedia_spi_rtc_read(rtc->regs, RTC_H_COUNT, &hour);
	ret |= (unsigned int)xmedia_spi_rtc_read(rtc->regs, RTC_D_COUNT_L, &dayl);
	ret |= (unsigned int)xmedia_spi_rtc_read(rtc->regs, RTC_D_COUNT_H, &dayh);
...
static int xmedia_rtc_set_time(struct device *dev, struct rtc_time *time)
{
	...
	ret |= (unsigned int)xmedia_spi_rtc_write(rtc->regs, RTC_LR_10MS, 0);
	ret |= (unsigned int)xmedia_spi_rtc_write(rtc->regs, RTC_LR_S, time->tm_sec);
	ret |= (unsigned int)xmedia_spi_rtc_write(rtc->regs, RTC_LR_M, time->tm_min);
	ret |= (unsigned int)xmedia_spi_rtc_write(rtc->regs, RTC_LR_H, time->tm_hour);
	ret |= (unsigned int)xmedia_spi_rtc_write(rtc->regs, RTC_LR_D_L, (days & 0xFF));
	ret |= (unsigned int)xmedia_spi_rtc_write(rtc->regs, RTC_LR_D_H, (days >> 8));
				...
				struct rtc_wkalrm *alrm)
{
	...
	ret |= (unsigned int)xmedia_spi_rtc_read(rtc->regs, RTC_MR_S, &second);
	ret |= (unsigned int)xmedia_spi_rtc_read(rtc->regs, RTC_MR_M, &minute);
	ret |= (unsigned int)xmedia_spi_rtc_read(rtc->regs, RTC_MR_H, &hour);
	ret |= (unsigned int)xmedia_spi_rtc_read(rtc->regs, RTC_MR_D_L, &dayl);
	ret |= (unsigned int)xmedia_spi_rtc_read(rtc->regs, RTC_MR_D_H, &dayh);
	...
	ret |= (unsigned int)xmedia_spi_rtc_read(rtc->regs, RTC_IMSC, &int_state);
	if (ret) {
		dev_err(dev, "IO err.\n");
		return ret;
	}
...
static int xmedia_rtc_set_alarm(struct device *dev, struct rtc_wkalrm *alrm)
{
	...
	ret |= (unsigned int)xmedia_spi_rtc_write(rtc->regs, RTC_MR_10MS, 0);
	ret |= (unsigned int)xmedia_spi_rtc_write(rtc->regs, RTC_MR_S, alrm->time.tm_sec);
	ret |= (unsigned int)xmedia_spi_rtc_write(rtc->regs, RTC_MR_M, alrm->time.tm_min);
	ret |= (unsigned int)xmedia_spi_rtc_write(rtc->regs, RTC_MR_H, alrm->time.tm_hour);
	ret |= (unsigned int)xmedia_spi_rtc_write(rtc->regs, RTC_MR_D_L, (days & 0xFF));
	ret |= (unsigned int)xmedia_spi_rtc_write(rtc->regs, RTC_MR_D_H, (days >> 8));

	ret |= (unsigned int)xmedia_spi_rtc_read(rtc->regs, RTC_IMSC, &val);
	if (alrm->enabled)
		ret |= (unsigned int)xmedia_spi_rtc_write(rtc->regs, RTC_IMSC,
					   val | AIE_INT_MASK);
	else
		ret |= (unsigned int)xmedia_spi_rtc_write(rtc->regs, RTC_IMSC,
					   val & ~AIE_INT_MASK);
...
static irqreturn_t xmedia_rtc_alm_interrupt(int irq, void *data)
{
	...
	ret |= (unsigned int)xmedia_spi_read(rtc->regs, RTC_INT, &val);
	ret |= (unsigned int)xmedia_spi_write(rtc->regs, RTC_INT_CLR, AIE_INT_MASK);
			   ...
			   unsigned int cmd, unsigned long arg)
{
	...
	case RTC_PLL_SET: {
		...
		ret |= (unsigned int)xmedia_spi_rtc_write(rtc->regs, RTC_FREQ_H, freq_h);
		ret |= (unsigned int)xmedia_spi_rtc_write(rtc->regs, RTC_FREQ_L, freq_l);
	...
	case RTC_PLL_GET: {
		...
		ret |= (unsigned int)xmedia_spi_rtc_read(rtc->regs, RTC_FREQ_H, &freq_h);
		ret |= (unsigned int)xmedia_spi_rtc_read(rtc->regs, RTC_FREQ_L, &freq_l);
...
static int xmedia_rtc_init(struct xmedia_rtc *rtc)
{
	...
	ret = (unsigned int)xmedia_spi_rtc_read(spi_reg, RTC_SAR_CTRL, &val);
	if (ret) {
		dev_err(&rtc->rtc_dev->dev, "IO err.\n");
		return ret;
	}
	val = val & (~ BIT(5));
	ret |= (unsigned int)xmedia_spi_rtc_write(spi_reg, RTC_SAR_CTRL, val);
	ret |= (unsigned int)xmedia_spi_rtc_write(spi_reg, RTC_IMSC, INT_MSK_DEFAULT);
	...
	ret |= (unsigned int)xmedia_spi_rtc_write(spi_reg, RTC_REG_LOCK4, 0x5A);
	ret |= (unsigned int)xmedia_spi_rtc_write(spi_reg, RTC_REG_LOCK3, 0x5A);
	ret |= (unsigned int)xmedia_spi_rtc_write(spi_reg, RTC_REG_LOCK2, 0xAB);
	ret |= (unsigned int)xmedia_spi_rtc_write(spi_reg, RTC_REG_LOCK1, 0xCD);
#endif

	/*driver capability */
	ret |= (unsigned int)xmedia_spi_rtc_write(spi_reg, RTC_CLK_CFG, 0x01);

	/* default FREQ COEF */
	ret |= (unsigned int)xmedia_spi_rtc_write(spi_reg, RTC_FREQ_H, FREQ_H_DEFAULT);
	ret |= (unsigned int)xmedia_spi_rtc_write(spi_reg, RTC_FREQ_L, FREQ_L_DEFAULT);

	ret |= (unsigned int)xmedia_spi_rtc_read(spi_reg, RTC_INT_RAW, &val);
	if (ret) {
		dev_err(&rtc->rtc_dev->dev, "IO err.\n");
		return ret;
	}
	...
	if (val & LV_INT_MASK) {
		...
		xmedia_spi_write(rtc->regs, RTC_INT_CLR, 1);
	}
...
#ifdef CONFIG_PM
static int xmedia_rtc_suspend(struct device *dev)
{
	struct platform_device *pdev = to_platform_device(dev);
	struct xmedia_rtc *rtc = platform_get_drvdata(pdev);

	if (rtc->rtc_irq >= 0) {
		if (device_may_wakeup(&pdev->dev))
			enable_irq_wake(rtc->rtc_irq);
		else
			disable_irq_wake(rtc->rtc_irq);
	}

	return 0;
}

static int xmedia_rtc_resume(struct device *dev)
{
	struct platform_device *pdev = to_platform_device(dev);
	struct xmedia_rtc *rtc = platform_get_drvdata(pdev);

	if (rtc->rtc_irq >= 0 && device_may_wakeup(&pdev->dev))
		disable_irq_wake(rtc->rtc_irq);

	return 0;
}

static const struct dev_pm_ops xmedia_rtc_pm_ops = {
	.suspend = xmedia_rtc_suspend,
	.resume = xmedia_rtc_resume,
};

#define XMEDIA_RTC_PM_OPS (&xmedia_rtc_pm_ops)
#else
#define XMEDIA_RTC_PM_OPS NULL
#endif
