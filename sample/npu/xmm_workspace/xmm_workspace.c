...
typedef struct _ptheadargs{
    xmedia_cl_context context;
     int i;
     xmedia_cl_device_id *devices;
     char *nocache_buffer_start[1];
...
static int load_model_file_withmem_workspace(void * args){
    xmedia_cl_s32 k = 0;
    ...
    ptheadargs_t *args_t = (ptheadargs_t *)args;
    k = (xmedia_cl_s32)args_t->i;
    void *buff[DEFAULT_INOUT_NUM] = { 0 };
    xmedia_cl_s8 name[64];
    xmedia_cl_s32 output_size[DEFAULT_INOUT_NUM];
    xmedia_cl_s8 model[64];
    sprintf(model, "data/gnn%d.bin", k);
    xmedia_cl_s32 i = 0, j, size, ret;
    ...
    ret = xmedia_cl_init();
    if (ret != XMEDIA_CL_SUCCESS) {
        ret = xmedia_cl_uninit();
        if (ret != XMEDIA_CL_SUCCESS) {
            printf("xmedia_cl_uninit1 err, errno %d\n", ret);
            return ret;
        }
    ...
    ret = XMEDIA_API_SYS_MmzAlloc_Cached(&u64SrcPhyAddr[0],
                                  (void **)(&nocache_buffer_start[0]), NULL,
                                  NULL, worksize);
    if (XMEDIA_CL_SUCCESS != ret) {
        goto ERROR;
    }*/
    ...
    ret = XMEDIA_API_SYS_MmzAlloc_Cached(&u64SrcPhyAddr[1],
                                  (void **)(&nocache_buffer_start[1]), "npu_weight",
                                  NULL, weightsize);
    if (XMEDIA_CL_SUCCESS != ret) {
        goto ERROR;
    }
    printf("load_model_file_withmem_workspace, model:%s\n", model);

    ret = xmedia_cl_get_workspace_addr(context, (void **)&nocache_buffer_start[0], &worksize);
    if (XMEDIA_CL_SUCCESS != ret) {
        printf("xmedia_cl_get_workspace_addr, errno %d\n", ret);
        goto ERROR;
    }

    //printf("xmedia_cl_get_workspace_addr ,nocache_buffer_start[0]:0X%x, worksize:%d\n", (xmedia_cl_u32)nocache_buffer_start[0], worksize);
    ...
    for (i = 0; i < input.num; i++) {
        ...
        sprintf(name, "data/input_data%d_%d.bin", k, i);
    ...
    for (i = 0; i < output.num; i++) {
        ...
        sprintf(name, "data/output_data%d_%d.bin", k, i);
    ...
    for (j = 0; j < 2; j++) {
        printf("xmedia_cl_graph_pocess:%d, model:%s\n", j, model);
        //执行推理
...
ERROR:
    ...
    if (nocache_buffer_start[2] != NULL) {
        XMEDIA_API_SYS_MmzFree(u64SrcPhyAddr[2], nocache_buffer_start[2]);
    }

    if (nocache_buffer_start[3] != NULL) {
        XMEDIA_API_SYS_MmzFree(u64SrcPhyAddr[3], nocache_buffer_start[3]);
    }

    if (nocache_buffer_start[1] != NULL) {
        XMEDIA_API_SYS_MmzFree(u64SrcPhyAddr[1], nocache_buffer_start[1]);
    }
    ...
    if (graph != NULL) {
        ret = xmedia_cl_graph_unload(graph);
        if (ret != XMEDIA_CL_SUCCESS) {
            printf("xmedia_cl_graph_unload1 err, errno %d\n", ret);
            return ret;
        }
    }

    if (context != NULL) {
        printf("xmedia_cl_release_context1_start\n");
        ret = xmedia_cl_release_context(context);
        if (ret != XMEDIA_CL_SUCCESS) {
            printf("xmedia_cl_release_context1 err, errno %d\n", ret);
            return ret;
        }
        printf("xmedia_cl_release_context1_end\n");
    }

    if (devices != NULL) {
        ret = xmedia_cl_release_device_ids(devices, &num_devices);
        if (ret != XMEDIA_CL_SUCCESS) {
            printf("xmedia_cl_release_device_ids1 err, errno %d\n", ret);
            return ret;
        }
        free(devices);
    }
...
int main()
{
    ...
    xmedia_cl_u32 worksize_1=0, weightsize_1=0;
    xmedia_cl_u32 worksize_2=0, weightsize_2=0;
    xmedia_u64 u64SrcPhyAddr[1] = {0};
    xmedia_char *nocache_buffer_start[1] = {0};
   ...
   //公共模块初始化
    ...
    xmedia_cl_graph_querysize_from_file("data/gnn0.bin", &worksize, &weightsize);
    xmedia_cl_graph_querysize_from_file("data/gnn1.bin", &worksize_1, &weightsize_1);
    xmedia_cl_graph_querysize_from_file("data/gnn2.bin", &worksize_2, &weightsize_2);
    worksize = worksize > worksize_1 ? worksize : worksize_1;
    worksize = worksize > worksize_2 ? worksize : worksize_2;

    ret = XMEDIA_API_SYS_MmzAlloc_Cached(&u64SrcPhyAddr[0],
                                  (void **)(&nocache_buffer_start[0]), "npu_workspace",
                                  NULL, worksize);
    if (XMEDIA_CL_SUCCESS != ret) {
        goto ERROR;
    }

    ret = xmedia_cl_set_workspace_addr(context, (void *)(nocache_buffer_start[0]), worksize);
    if (XMEDIA_CL_SUCCESS != ret) {
        printf("xmedia_cl_set_workspace_addr err, errno %d\n", ret);
        goto ERROR;
    }

    args[0].i = 0;
    ret = pthread_create(&tid[0], NULL,(void *)load_model_file_withmem_workspace, (void *)&args[0]);
    if(ret) {
        printf("create pthread wait_job19 [%d] err\n", ret);
    }
    
    args[1].i = 1;
    ret = pthread_create(&tid[1], NULL,(void *)load_model_file_withmem_workspace, (void *)&args[1]);
    if(ret) {
        printf("create pthread wait_job20 [%d] err\n", ret);
    }

    args[2].i = 2;
    ret = pthread_create(&tid[2], NULL,(void *)load_model_file_withmem_workspace, (void *)&args[2]);
    if(ret) {
        printf("create pthread wait_job21 [%d] err\n", ret);
    }

    ret = pthread_join(tid[0], NULL);
    if(ret) {
        printf("pthread join0 [%d] err\n", ret);
    }
    
    ret = pthread_join(tid[1], NULL);
    if(ret) {
        printf("pthread join01 [%d] err\n", ret);
    }

    ret = pthread_join(tid[2], NULL);
    if(ret) {
        printf("pthread join02 [%d] err\n", ret);
    }
ERROR:
    if (nocache_buffer_start[0] != NULL) {
        XMEDIA_API_SYS_MmzFree(u64SrcPhyAddr[0], nocache_buffer_start[0]);
    }

    if (context != NULL) {
        ret = xmedia_cl_release_context(context);
        if (ret != XMEDIA_CL_SUCCESS) {
            printf("xmedia_cl_release_context2 err, errno %d\n", ret);
            return ret;
        }
    }
