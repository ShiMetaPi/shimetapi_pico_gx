...
static int fmc100_host_init(struct fmc_host *host)
{
	...
	if ((reg & OP_MODE_MASK) == OP_MODE_BOOT) {
		...
		fmc_pr(BT_DBG, "\t||||-Controller enter normal mode from boot mode\n");
...
void spi_lock_update_address(struct fmc_host *host)
{
	...
	case MID_ESMT:
		...
		if (host->level != lock_level_max)
			host->end_addr = chipsize - (erasesize  << (lock_level_max - host->level));
		else
			host->end_addr = chipsize;
		return;
	case MID_CFEON:
		if (host->level != lock_level_max)
			host->end_addr = chipsize - (erasesize << (host->level - 1));
		else
			host->end_addr = chipsize;
		return;
	default:
