...
xmedia_s32 rcnn_init(xmedia_void *context, const xmedia_svp_task_cfg cfg)
{
    ...
    svp_rcnn_context *rcnn_context = (svp_rcnn_context *)context;
    ...
    rcnn_context->detect_info->w = INPUT_DEFAULT_WIDE;
    rcnn_context->detect_info->h = INPUT_DEFAULT_HIGH;
    ...
    CHECK_SVP_COND_GOTO_ERROR(ret == XMEDIA_FAILURE, EXIT4, "img_resize mmz alloc fail !\n");
    return ret;
...
xmedia_s32 rcnn_uninit(xmedia_void *context)
{
    ...
    if (rcnn_context->resize_phyaddr != XMEDIA_NULL) {
...
xmedia_s32 rcnn_process(xmedia_void *context, const xmedia_svp_task_input *input, xmedia_svp_yolov5_output *output)
{
    xmedia_s32 ret = XMEDIA_SUCCESS;
    xmedia_u8 i;
