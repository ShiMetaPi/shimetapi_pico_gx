...
static int lotus_fmc100_spinand_read_sr(struct flash_regop *drv)
{
	...
	u8 status = 0;
	struct flash_regop_info info = {0};
	struct lotus_fmc_xnand *xnand = container_of(drv, struct lotus_fmc_xnand, regop[0]);
...
static int lotus_fmc100_xnand_dev_ready(struct mtd_info *mtd)
{
	u8 status;
...
static void lotus_fmc100_xnand_cmdfunc(struct mtd_info * mtd, unsigned int command,
...
static void lotus_fmc100_xnand_select_chip(struct mtd_info *mtd, int chipselect)
{
	struct nand_chip *chip = (struct nand_chip *)mtd->priv;
	struct lotus_fmc_xnand *xnand = (struct lotus_fmc_xnand *)chip->priv;
...
static uint8_t lotus_fmc100_xnand_read_byte(struct mtd_info * mtd)
{
	u8 retval;
	int offset;
	struct nand_chip *chip = (struct nand_chip *)mtd->priv;
...
static void lotus_fmc100_xnand_read_buf(struct mtd_info *mtd, uint8_t *buf, int len)
{
	int offset;
	struct nand_chip *chip = (struct nand_chip *)mtd->priv;
	struct lotus_fmc_xnand *xnand = (struct lotus_fmc_xnand *)chip->priv;
...
static int lotus_fmc100_xnand_read_page(struct mtd_info *mtd, struct nand_chip *chip,
...
static void lotus_fmc100_xnand_write_buf(struct mtd_info *mtd, const uint8_t *buf,
				     int len)
{
	int offset;
	struct nand_chip *chip = (struct nand_chip *)mtd->priv;
...
static int lotus_fmc100_xnand_write_page(struct mtd_info *mtd, struct nand_chip *chip,
				       ...
				       struct lotus_fmc_xnand *xnand)
{
	...
	chip->cmdfunc = lotus_fmc100_xnand_cmdfunc;
	chip->dev_ready = lotus_fmc100_xnand_dev_ready;
	chip->select_chip = lotus_fmc100_xnand_select_chip;
	chip->read_byte = lotus_fmc100_xnand_read_byte;
	chip->read_buf = lotus_fmc100_xnand_read_buf;
	chip->write_buf = lotus_fmc100_xnand_write_buf;
	chip->IO_ADDR_R = chip->IO_ADDR_W = xnand->host->iobase;
	...
	ret = nand_scan_ident(mtd, LOTUS_FMC100_OP_CFG_NUM_CS, nand_id_table);
	if (ret)
		return ret;
	...
	if (chip->chipsize > (128 << 20)) /* 128,20 */
			 ...
			 int ifmode)
{
	...
	lotus_fmc100_xnand_select_chip(mtd, 0);
	xnand->cmd_reset(xnand);
	xnand->cmd_readid(xnand);
