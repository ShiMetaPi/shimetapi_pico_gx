...
static int atoul(const char *str, int * pulValue)
{
	int ulResult=0;
...
static int  atoulx(const char *str, int * pulValue)
{
	int   ulResult=0;
	char ch;
...
static int  StrToNumber(const char *str , int * pulValue)
{
...
int ssp_read(int argc, char* argv[])
{
	...
	unsigned int spi_num = 0, csn = 0, dev_addr = 0, reg_addr = 0, cur_reg_addr = 0;
	...
	printf("spi_num:%u, csn:%u\n"
			"dev_addr:0x%04x, reg_addr:0x%04x, num_reg:%u, "
			"dev_width:%u, reg_width:%u, data_width:%u, reg_order: %u, data_order: %u\n",
    ...
    mesg[0].tx_buf = (uintptr_t)buf;
    mesg[0].rx_buf = (uintptr_t)buf;
...
int ssp_write(int argc, char* argv[])
{
	...
	unsigned char  buf[SSP_BUF_LEN];
	struct spi_ioc_transfer mesg[1];
	unsigned int spi_num = 0, csn = 0, dev_addr = 0, reg_addr = 0, data = 0;
