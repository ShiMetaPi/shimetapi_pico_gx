...
static void regop_dump_help(struct regop_intf_data *data)
{
	...
	count = snprintf(ptr, sz_buf, "Key Word: cmd=0xNNNN,nr_cmd=N,addrh=0xNN,addrl=0xNNNNNNNN,addr_cycle=N,dummy=N,sz_buf=N,rb,msleep=N,read/write,\n");
	sz_buf -= count;
	ptr += count;
...
static int regop_resolve_valset(int ix, struct flash_regop_info *info,
	char *buf, struct cmd_val_t *val)
{
	ulong value;
	if (ix == 2) { /* 2 : address */
		...
		memcpy(val->buf, &value, min_t(ulong, val->sz_buf, sizeof(value)));
	}
	return 0;
}

static void regop_resolve_bufcheck(char *buf, char *ptail)
{
	char chr;
	char *nextbuf = buf;
	...
	*op = 0;
	...
	while (buf < ptail) {
		...
		for (ix = 0; ix < ARRAY_SIZE(cmd_val); ix++) {
			...
			if ((int)access_ok(VERIFY_READ, (const void *)buf, val->sz_key) == 0)
				continue;

			if (memcmp(buf, val->key, val->sz_key))
				continue;

			if ((ix == 0 || ix == 1) && isalpha(buf[val->sz_key])) {
				continue;
			} else if (ix == 0 || ix == 1) {
				*op = val->key[0];
				buf += val->sz_key;
			...
			if (regop_resolve_valset(ix, info, buf, val) != 0)
				return NULL;
		...
		if (ix == ARRAY_SIZE(cmd_val))
			regop_resolve_bufcheck(buf, ptail);
	}
out:
	*msecs = _msecs;
...
static const struct proc_ops regop_fops = {
	.proc_open      = regop_open,
	.proc_read      = regop_read,
	.proc_lseek     = no_llseek,
	.proc_release   = regop_release,
	.proc_write     = regop_write,
};
#else
static const struct file_operations regop_fops = {
	.owner = THIS_MODULE,
	.open      = regop_open,
	.read      = regop_read,
	.llseek     = no_llseek,
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5, 0, 0)
static const struct proc_ops regop_data_fops = {
	.proc_read      = regop_data_read,
	.proc_write     = regop_data_write,
};

#else
static const struct file_operations regop_data_fops = {
	.owner = THIS_MODULE,
