...
#include <linux/lotus/flash_stats.h>
...
#define EPM(_oobbuf, _epmoffset) (*(u16 *)((u8 *)_oobbuf + _epmoffset))
#define BBM(_oobbuf) (*(u8 *)_oobbuf)

/******************************************************************************/

static void __maybe_unused nand_register_dump(struct lotus_fmc_host *host)
{
	int ix;
	...
	for (ix = 0; ix <= 0xF0; ix += 0x04) {
		...
		printk("%08X ", lotus_fmc_read(host, ix));
	}
	printk("\n");
}
/******************************************************************************/

static int lotus_fmc100_xnand_wait_device_ready(struct flash_regop *regop)
{
	int status;
	ulong deadline;
...
static u8 *lotus_fmc100_spinand_read_raw_bbm(struct lotus_fmc_xnand *xnand)
{
	struct lotus_fmc_host *lotus_fmc = xnand->host;
	...
	return (u8 *)lotus_fmc->iobase;
}
/*****************************************************************************/

static u8 *lotus_fmc100_nand_read_raw_bbm(struct lotus_fmc_xnand *xnand)
{
	struct lotus_fmc_host *lotus_fmc = xnand->host;
	struct cmdfunc_data *cmdfunc = xnand->cmdfunc;
	struct flash_regop_info info = {0};
...
static int lotus_fmc100_spinand_read_sr(struct flash_regop *drv)
{
	u8 retsr = 0;
	u8 status = 0;
	struct flash_regop_info info = {0};
...
static int lotus_fmc100_nand_read_sr(struct flash_regop *drv)
{
	u8 status = 0;
	struct flash_regop_info info = {0};
...
static int lotus_fmc100_xnand_dev_ready(struct nand_chip *chip)
{
	u8 status;
	struct lotus_fmc_xnand *xnand = (struct lotus_fmc_xnand *)chip->priv;
...
static bool lotus_fmc100_xnand_is_page_empty(struct lotus_fmc_xnand *xnand,
					 u8 oob_bitsflip)
{
	u16 epm = EPM(xnand->oobbuf, xnand->epmoffset);
...
static bool lotus_fmc100_xnand_is_bad_block(struct lotus_fmc_xnand *xnand,
					u8 oob_bitsflip)
{
...
static inline int lotus_fmc100_xnand_dma_map(struct lotus_fmc_xnand *xnand,
					 ...
					 enum dma_data_direction direction)
{
	...
	lotus_fmc_write(lotus_fmc, (u32)page->dma_pagebuf, dmaaddr_reg);

	if (xnand->pagesize >= SZ_8K) {
		u32 phyaddr = (u32)page->dma_pagebuf + SZ_4K;

		lotus_fmc_write(lotus_fmc, phyaddr, dmaaddr_reg + 0x4);

		if (xnand->pagesize >= SZ_16K) {
			lotus_fmc_write(lotus_fmc, phyaddr + SZ_4K, dmaaddr_reg + 0x8);
			lotus_fmc_write(lotus_fmc, phyaddr + SZ_8K, dmaaddr_reg + 0xC);
		}
	}
	...
	lotus_fmc_write(lotus_fmc, (u32)page->dma_oobbuf, dmaaddr_reg + 0x10);

	return 0;
}
/******************************************************************************/

static int lotus_fmc100_xnand_dma_transfer(struct lotus_fmc_xnand *xnand,
				       enum dma_data_direction direction)
{
	int ret;
	u32 regval;
	...
	ret = lotus_fmc100_xnand_dma_map(xnand, &page, LOTUS_FMC100_DMA_SADDR_D0,
		direction);
	if (ret)
		return ret;

	if (xnand->ecc_strength == 0)
		lotus_fmc_write(lotus_fmc, LOTUS_FMC100_DMA_LEN_MASK(xnand->oobsize),
			LOTUS_FMC100_DMA_LEN);
	...
	/* set randomizer */
	regval = lotus_fmc_read(lotus_fmc, LOTUS_FMC100_GLOBAL_CFG);
	...
	/* set address */
	lotus_fmc_write(lotus_fmc, cmdfunc->page_addr << 16, LOTUS_FMC100_ADDRL);
	lotus_fmc_write(lotus_fmc, cmdfunc->page_addr >> 16, LOTUS_FMC100_ADDRH);

	/* set command */
	lotus_fmc_write(lotus_fmc, reg->cmd, LOTUS_FMC100_CMD);

	lotus_fmc->irq_enable(lotus_fmc);

	lotus_fmc_write(lotus_fmc, reg->op_ctrl, LOTUS_FMC100_OP_CTRL);
...
static int lotus_fmc100_set_nand_fast_timing(struct lotus_fmc_xnand *nand)
{
	u32 regval;
	struct lotus_fmc_host *lotus_fmc = nand->host;

	lotus_fmc_write(lotus_fmc, nand->reg.fmc_pnd_pwidth_cfg,
		LOTUS_FMC100_PND_PWIDTH_CFG);

	regval = lotus_fmc_read(lotus_fmc, LOTUS_FMC100_GLOBAL_CFG);
	regval |= LOTUS_FMC100_GLOBAL_CFG_EDO_MODE;
	lotus_fmc_write(lotus_fmc, regval, LOTUS_FMC100_GLOBAL_CFG);

	lotus_fmc_write(lotus_fmc, LOTUS_FMC100_READ_TIMING_TUNE_VALUE,
		    LOTUS_FMC100_READ_TIMING_TUNE);
	return 0;
}
/******************************************************************************/

static int lotus_fmc100_set_nand_slow_timing(struct lotus_fmc_xnand *nand)
{
	u32 regval;
	struct lotus_fmc_host *lotus_fmc = nand->host;

	lotus_fmc_write(lotus_fmc, 0xFFF, LOTUS_FMC100_PND_PWIDTH_CFG);

	regval = lotus_fmc_read(lotus_fmc, LOTUS_FMC100_GLOBAL_CFG);
	regval &= ~LOTUS_FMC100_GLOBAL_CFG_EDO_MODE;
	lotus_fmc_write(lotus_fmc, regval, LOTUS_FMC100_GLOBAL_CFG);

	lotus_fmc_write(lotus_fmc, 0, LOTUS_FMC100_READ_TIMING_TUNE);

	return 0;
}
/******************************************************************************/

static int lotus_fmc100_xnand_do_read_retry(struct lotus_fmc_xnand *xnand)
{
	int retry_mode = 1;
	 ...
	 */
	lotus_fmc100_set_nand_slow_timing(xnand);
	...
	while (retry_mode < xnand->read_retry->count) {
		...
		lotus_fmc100_set_nand_fast_timing(xnand);
		lotus_fmc100_xnand_dma_transfer(xnand, DMA_FROM_DEVICE);
		lotus_fmc100_set_nand_slow_timing(xnand);

		/* ecc success */
		regval = lotus_fmc_read(xnand->host, LOTUS_FMC100_INT);
		if (!(regval & LOTUS_FMC100_INT_ERR_INVALID))
			break;
...
static int lotus_fmc100_xnand_fix_read(struct lotus_fmc_xnand *xnand)
{
	u8 ecc[16] = {0};
	int max_bitsflip = 0;
	...
	if (xnand->cmdfunc->command == NAND_CMD_READOOB) {
		xnand->is_bad_block = lotus_fmc100_xnand_is_bad_block(xnand,
	...
	xnand->is_empty_page = lotus_fmc100_xnand_is_page_empty(xnand,
		ecc[xnand->ctrl_info->nr_ecc_sect - 1]);
	if (xnand->is_empty_page)
		return 0;

	if (max_bitsflip > xnand->ecc_strength && xnand->read_retry) {
		lotus_fmc100_xnand_do_read_retry(xnand);
		xnand->ctrl_info->dump(xnand, ecc, &max_bitsflip);
	}
...
static void lotus_fmc100_spinand_cmd_readid(struct lotus_fmc_xnand *xnand)
{
	u8 id[FLASH_ID_LEN];
	static int cs[2] = {0};
	struct flash_regop_info info = {0};
...
static void lotus_fmc100_nand_cmd_readid(struct lotus_fmc_xnand *xnand)
{
	u8 id[FLASH_ID_LEN];
	static int cs[2] = {0};
	struct flash_regop_info info = {0};
...
static void lotus_fmc100_nand_cmd_reset(struct lotus_fmc_xnand *xnand)
{
	struct flash_regop_info info = {0};
	struct lotus_fmc_host *host = xnand->host;
...
static void lotus_fmc100_spinand_cmd_reset(struct lotus_fmc_xnand *xnand)
{
	struct flash_regop_info info = {0};
...
static void lotus_fmc100_nand_cmd_erase(struct lotus_fmc_xnand *xnand)
{
	struct flash_regop_info info = {0};
...
static void lotus_fmc100_spinand_cmd_erase(struct lotus_fmc_xnand *xnand)
{
	struct flash_regop_info info = {0};
...
static void __maybe_unused lotus_fmc100_spinand_reg_read(struct lotus_fmc_xnand *xnand)
{
	u32 regval;
	struct cmdfunc_data *cmdfunc = xnand->cmdfunc;
	struct lotus_fmc_host *lotus_fmc = xnand->host;
	...
	regval &= ~LOTUS_FMC100_CFG_ECC_TYPE_MASK;
	regval |= xnand->reg.fmc_cfg_ecc_type;
	regval &= ~LOTUS_FMC100_CFG_PAGE_SIZE_MASK;
	...
	regval = LOTUS_FMC100_OP_CFG_ADDR_NUM(2);
	regval |= LOTUS_FMC100_OP_CFG_DUMMY_NUM(1);
	if (lotus_fmc->chipselect)
		regval |= LOTUS_FMC100_OP_CFG_FM_CS;
	regval |= LOTUS_FMC100_OP_CFG_OEN_EN;
	lotus_fmc_write(lotus_fmc, regval, LOTUS_FMC100_OP_CFG);
	...
	lotus_fmc_write(lotus_fmc, SPINAND_OP_READ, LOTUS_FMC100_CMD);

	regval = LOTUS_FMC100_OP_REG_OP_START |
		LOTUS_FMC100_OP_DUMMY_EN |
		LOTUS_FMC100_OP_CMD1_EN |
		LOTUS_FMC100_OP_READ_DATA_EN |
		LOTUS_FMC100_OP_ADDR_EN;

	lotus_fmc_write(lotus_fmc, regval, LOTUS_FMC100_OP);
	lotus_fmc->wait_host_ready(lotus_fmc);
}
/******************************************************************************/

static void lotus_fmc100_xnand_read_cache(struct lotus_fmc_xnand *xnand, int page_addr)
{
	int ret;
	char *pagebuf, *oobbuf;
	...
	case FLASHCACHE_DISABLE:
		lotus_fmc100_xnand_dma_transfer(xnand, DMA_FROM_DEVICE);
		lotus_fmc100_xnand_fix_read(xnand);
		break;
...
static void lotus_fmc100_xnand_cmdfunc(struct nand_chip *chip, unsigned int command,
				   int column, int page_addr)
{
	...
	case NAND_CMD_READ0:
		...
		lotus_fmc100_xnand_read_cache(xnand, cmdfunc->page_addr);
	...
	case NAND_CMD_PAGEPROG:
		EPM(xnand->oobbuf, xnand->epmoffset) = 0;

		lotus_fmc100_xnand_dma_transfer(xnand, DMA_TO_DEVICE);
...
static void lotus_fmc100_xnand_select_chip(struct nand_chip *chip, int chipselect)
{
	struct lotus_fmc_xnand *xnand = (struct lotus_fmc_xnand *)chip->priv;
...
static uint8_t lotus_fmc100_xnand_read_byte(struct nand_chip *chip)
{
	u8 retval;
	int offset;
	struct lotus_fmc_xnand *xnand = (struct lotus_fmc_xnand *)chip->priv;
...
static void lotus_fmc100_xnand_read_buf(struct nand_chip *chip, uint8_t *buf, int len)
{
	int offset;
	struct lotus_fmc_xnand *xnand = (struct lotus_fmc_xnand *)chip->priv;
	struct cmdfunc_data *cmdfunc = xnand->cmdfunc;
...
static int lotus_fmc100_xnand_read_page(struct nand_chip *chip,
...
static void lotus_fmc100_xnand_write_buf(struct nand_chip *chip, const uint8_t *buf,
				     int len)
{
	int offset;
	struct lotus_fmc_xnand *xnand = (struct lotus_fmc_xnand *)chip->priv;
...
static int lotus_fmc100_xnand_write_page(struct nand_chip *chip,
...
static u32 lotus_fmc100_ecc_strength_reg(u32 value, bool _to_reg)
{
	int ix;
	u32 ecc_strength_reg[] = {
		0,  LOTUS_FMC100_CFG_ECC_TYPE_NONE,
		8,  LOTUS_FMC100_CFG_ECC_TYPE_8BIT,
		16, LOTUS_FMC100_CFG_ECC_TYPE_16BIT,
		24, LOTUS_FMC100_CFG_ECC_TYPE_24BIT,
		28, LOTUS_FMC100_CFG_ECC_TYPE_28BIT,
		40, LOTUS_FMC100_CFG_ECC_TYPE_40BIT,
		64, LOTUS_FMC100_CFG_ECC_TYPE_64BIT,
	};
...
static u32 lotus_fmc100_pagesize_reg(u32 value, bool _to_reg)
{
	int ix;
	u32 pagesize_reg[] = {
		SZ_2K,  LOTUS_FMC100_CFG_PAGE_SIZE_2K,
		SZ_4K,  LOTUS_FMC100_CFG_PAGE_SIZE_4K,
		SZ_8K,  LOTUS_FMC100_CFG_PAGE_SIZE_8K,
		SZ_16K, LOTUS_FMC100_CFG_PAGE_SIZE_16K,
	};
...
u32 lotus_fmc100_syncmode_reg(u32 value, u32 _to_reg)
{
	int ix;
	u32 syncmode_reg[] = {
		NAND_MODE_SYNC_ONFI23,  LOTUS_FMC100_CFG_NFMODE_ONFI23,
		NAND_MODE_SYNC_ONFI30,  LOTUS_FMC100_CFG_NFMODE_ONFI30,
		NAND_MODE_SYNC_TOGGLE10, LOTUS_FMC100_CFG_NFMODE_TOGGLE10,
		NAND_MODE_SYNC_TOGGLE20, LOTUS_FMC100_CFG_NFMODE_TOGGLE20,
	};
...
static void lotus_fmc100_dump_bitflip(void *host, u8 ecc[16], int *max_bitsflip)
{
	u32 *e = (u32 *)ecc;
	struct lotus_fmc_xnand *xnand = (struct lotus_fmc_xnand *)host;
	struct lotus_fmc_host *lotus_fmc = xnand->host;

	e[0] = lotus_fmc_read(lotus_fmc, LOTUS_FMC100_FMC_ERR_NUM0_BUF0);

	if (xnand->pagesize >= SZ_8K) {
		e[1] = lotus_fmc_read(lotus_fmc, LOTUS_FMC100_FMC_ERR_NUM0_BUF0 + 0x4);

		if (xnand->pagesize >= SZ_16K) {
			e[2] = lotus_fmc_read(lotus_fmc, LOTUS_FMC100_FMC_ERR_NUM0_BUF0 + 0x8);
			e[3] = lotus_fmc_read(lotus_fmc, LOTUS_FMC100_FMC_ERR_NUM0_BUF0 + 0xC);
		}
	}
...
#ifdef CONFIG_LOTUS_NAND_FS_MAY_NO_YAFFS2
static int nand_ooblayout_ecc_2k16bit(struct mtd_info *mtd, int section,
		  struct mtd_oob_region *oobregion)
{
	if (section)
		return -ERANGE;
...
static int nand_ooblayout_free_2k16bit(struct mtd_info *mtd, int section,
		  struct mtd_oob_region *oobregion)
{
	if (section)
		return -ERANGE;
...
static struct mtd_ooblayout_ops g_fmc100_xnand_ooblayout_2k16bit = {
	.ecc = nand_ooblayout_ecc_2k16bit,
	.free = nand_ooblayout_free_2k16bit,
};

static int nand_ooblayout_ecc_4k16bit(struct mtd_info *mtd, int section,
		  struct mtd_oob_region *oobregion)
{
	if (section)
		return -ERANGE;
...
static int nand_ooblayout_free_4k16bit(struct mtd_info *mtd, int section,
		  struct mtd_oob_region *oobregion)
{
	if (section)
		return -ERANGE;
...
static struct mtd_ooblayout_ops g_fmc100_xnand_ooblayout_4k16bit = {
	.ecc = nand_ooblayout_ecc_4k16bit,
	.free = nand_ooblayout_free_4k16bit,
};
#endif

#define SET_CTRL_INFO(_pagesize, _ecc_strength, _sz_data, _sz_ecc, _threshold, _ooblayout) { \
	.pagesize = (_pagesize), \
	.ecc = { .strength = (_ecc_strength), .step = SZ_1K, }, \
	.section = { .sz_data = (_sz_data), .sz_ecc = (_sz_ecc), }, \
	.threshold = (_threshold), \
	.ooblayout = (_ooblayout), \
	.dump = lotus_fmc100_dump_bitflip, \
	.nr_ecc_sect = 0, }

/******************************************************************************/

static struct nand_ctrl_info lotus_fmc100_nand_ctrl_info[] = {
	SET_CTRL_INFO(SZ_2K, 8,  1040, 14, 7, &g_fmc100_xnand_ooblayout),
	SET_CTRL_INFO(SZ_2K, 16, 1028, 28, 14, &g_fmc100_xnand_ooblayout),
	SET_CTRL_INFO(SZ_2K, 24, 1040, 42, 21, &g_fmc100_xnand_ooblayout),
	SET_CTRL_INFO(SZ_2K, 28, 1040, 50, 25, &g_fmc100_xnand_ooblayout),

	SET_CTRL_INFO(SZ_4K, 8,  1032, 14, 7, &g_fmc100_xnand_ooblayout),
	SET_CTRL_INFO(SZ_4K, 16, 1028, 28, 14, &g_fmc100_xnand_ooblayout),
	SET_CTRL_INFO(SZ_4K, 24, 1032, 42, 21, &g_fmc100_xnand_ooblayout),
	SET_CTRL_INFO(SZ_4K, 28, 1032, 50, 25, &g_fmc100_xnand_ooblayout),

	SET_CTRL_INFO(SZ_8K, 24, 1028, 42, 21, &g_fmc100_xnand_ooblayout),
	SET_CTRL_INFO(SZ_8K, 28, 1028, 50, 25, &g_fmc100_xnand_ooblayout),
	SET_CTRL_INFO(SZ_8K, 40, 1028, 70, 34, &g_fmc100_xnand_ooblayout),
	SET_CTRL_INFO(SZ_8K, 64, 1028, 112, 58, &g_fmc100_xnand_ooblayout),

	SET_CTRL_INFO(SZ_16K, 40, 1026, 70, 34, &g_fmc100_xnand_ooblayout),
	SET_CTRL_INFO(SZ_16K, 64, 1026, 112, 58, &g_fmc100_xnand_ooblayout),

	{0},
};
/******************************************************************************/

static struct nand_ctrl_info lotus_fmc100_spinand_ctrl_info[] = {
	SET_CTRL_INFO(SZ_2K, 8,  1040, 14, 7, &g_fmc100_xnand_ooblayout),
#ifdef CONFIG_LOTUS_NAND_FS_MAY_NO_YAFFS2
	SET_CTRL_INFO(SZ_2K, 16, 1028, 28, 14, &g_fmc100_xnand_ooblayout_2k16bit),
#endif
	SET_CTRL_INFO(SZ_2K, 24, 1040, 42, 21, &g_fmc100_xnand_ooblayout),

	SET_CTRL_INFO(SZ_4K, 8,  1032, 14, 7, &g_fmc100_xnand_ooblayout),
#ifdef CONFIG_LOTUS_NAND_FS_MAY_NO_YAFFS2
	SET_CTRL_INFO(SZ_4K, 16, 1028, 28, 14, &g_fmc100_xnand_ooblayout_4k16bit),
#endif
	SET_CTRL_INFO(SZ_4K, 24, 1032, 42, 21, &g_fmc100_xnand_ooblayout),
...
static int lotus_fmc100_xnand_sanely_check(struct mtd_info *mtd, struct nand_chip *chip,
				       struct lotus_fmc_xnand *xnand)
{
	...
	if (xnand->pagesize == SZ_2K && xnand->ecc_strength <= 8)
		pr_info("ECC:%dbits/512 ", xnand->ecc_strength >> 1);
	else
		pr_info("ECC:%dbits/1K ", xnand->ecc_strength);

	pr_info("Chip:%lldB ", mtd->size);

	if (xnand->ifmode == LOTUS_FMC_IFMODE_SPINAND)
		pr_info("maxclk:%luM ", xnand->clk_rate / 2000000);

	if (xnand->read_retry)
		pr_info("read-retry ");

	if (xnand->host->flags & NAND_MODE_SYNC_ONFI23)
		pr_info("ONFI DDR 2.3 ");
	else if (xnand->host->flags & NAND_MODE_SYNC_ONFI30)
		pr_info("ONFI DDR 3.0 ");
	else if (xnand->host->flags & NAND_MODE_SYNC_TOGGLE10)
		pr_info("Toggle DDR 1.0 ");
	else if (xnand->host->flags & NAND_MODE_SYNC_TOGGLE20)
		pr_info("Toggle DDR 2.0 ");
...
static int lotus_fmc100_xnand_mtd_erase(struct mtd_info *mtd, struct erase_info *instr)
{
	int ret;
...
static int lotus_fmc100_xnand_mtd_panic_write(struct mtd_info *mtd, loff_t to,
...
static int lotus_fmc100_xnand_mtd_read_oob(struct mtd_info *mtd, loff_t from,
...
static int lotus_fmc100_xnand_mtd_write_oob(struct mtd_info *mtd, loff_t to,
					struct mtd_oob_ops *ops)
{
	int ret;
	struct nand_chip *chip = (struct nand_chip *)mtd->priv;
...
static int lotus_fmc100_xnand_mtd_block_isbad(struct mtd_info *mtd, loff_t ofs)
{
	int ret;
	struct nand_chip *chip = (struct nand_chip *)mtd->priv;
	struct lotus_fmc_xnand *xnand = (struct lotus_fmc_xnand *)chip->priv;
...
static int lotus_fmc100_xnand_mtd_block_markbad(struct mtd_info *mtd, loff_t ofs)
{
	int ret;
	struct nand_chip *chip = (struct nand_chip *)mtd->priv;
	struct lotus_fmc_xnand *xnand = (struct lotus_fmc_xnand *)chip->priv;
...
static int lotus_fmc100_xnand_regop_open(void *priv)
{
	struct lotus_fmc_xnand *xnand = (struct lotus_fmc_xnand *)priv;
	struct lotus_fmc_host *lotus_fmc = xnand->host;

	mutex_lock(&lotus_fmc->lock);
	lotus_fmc->set_ifmode(lotus_fmc, xnand->ifmode);
...
static int lotus_fmc100_xnand_regop_release(void *priv)
{
	struct lotus_fmc_xnand *xnand = (struct lotus_fmc_xnand *)priv;
	struct lotus_fmc_host *lotus_fmc = xnand->host;
...
static int lotus_fmc100_nand_read_mp_start(struct lotus_fmc_xnand *nand, int page_addr,
				       int nr_pages)
{
	u32 regval;
	int fail_addr;
	struct lotus_fmc_host *lotus_fmc = nand->host;
	struct lotus_fmc_reg *reg = &nand->reg.read;
	...
	/* set address */
	lotus_fmc_write(lotus_fmc, page_addr << 16, LOTUS_FMC100_ADDRL);
	lotus_fmc_write(lotus_fmc, page_addr >> 16, LOTUS_FMC100_ADDRH);

	regval = (LOTUS_FMC100_FMC_CACHE_PAGE_NUM_MASK & nr_pages) |
		LOTUS_FMC100_FMC_CACHE_EXIT_OP_EN;
	lotus_fmc_write(lotus_fmc, regval, LOTUS_FMC100_FMC_CACHE_CTRL);

	lotus_fmc->irq_enable(lotus_fmc);

	regval = LOTUS_FMC100_OP_CTRL_DMA_OP_READY |
		LOTUS_FMC100_OP_CTRL_RD_OP_SEL_CACHE;
	lotus_fmc_write(lotus_fmc, regval, LOTUS_FMC100_OP_CTRL);
	...
	regval = lotus_fmc_read(lotus_fmc, LOTUS_FMC100_FMC_ECC_INVALID_ADDRL);
	fail_addr = ((regval & 0xFFFF0000) >> 16);

	regval = lotus_fmc_read(lotus_fmc, LOTUS_FMC100_FMC_ECC_INVALID_ADDRH);
...
static int lotus_fmc100_nand_read_mp_prepare(struct lotus_fmc_xnand *nand,
					 int page_addr, struct page_info *pages)
{
	...
	for (ix = 0; ix < 8; ix++) {
		...
		if (!ix)
			reg_addr = LOTUS_FMC100_DMA_SADDR_D0;
		else
			reg_addr = LOTUS_FMC100_FMC_DMA_SADDR_D0_P1 +
				(ix - 1) * 0x14;

		page->pagesize = nand->pagesize;
		page->oobsize = 32;

		ret = lotus_fmc100_xnand_dma_map(nand, page, reg_addr,
			DMA_FROM_DEVICE);
		if (ret) {
...
static int lotus_fmc100_nand_read_mp_finish(struct lotus_fmc_xnand *nand,
					...
					int count)
{
	...
	discard = nand->host->intr_status & LOTUS_FMC100_INT_ERR_INVALID;
	...
	for (ix = 0; ix < count; ix++) {
		...
		if (EPM(page->oobbuf, nand->epmoffset) == 0xFFFF) {
...
static int lotus_fmc100_nand_read_ahead(void *arg, int pageindex, int *skip_pages)
{
	int count;
	int status;
	struct page_info pages[9];

	struct lotus_fmc_xnand *nand = (struct lotus_fmc_xnand *)arg;
	struct lotus_fmc_host *lotus_fmc = nand->host;
	struct flash_cache *cache = nand->cache;

	mutex_lock(&lotus_fmc->lock);
	lotus_fmc->set_ifmode(lotus_fmc, nand->ifmode);
	nand->chip->legacy.select_chip(nand->chip, 0);

	count = lotus_fmc100_nand_read_mp_prepare(nand, pageindex, pages);
	if (count) {
		int fail_addr = lotus_fmc100_nand_read_mp_start(nand, pageindex,
			count);
		lotus_fmc100_nand_read_mp_finish(nand, fail_addr, pages, count);
	}
...
static void lotus_fmc100_spinand_set_xfer(struct lotus_fmc_xnand *xnand)
{
	struct spinand_chip_xfer *xfer;
	struct lotus_fmc_reg *read, *write;
	int rdatapin, wdatapin;

	xfer = spinand_get_xfer(g_fmc100_spinand_chip_xfer,
		xnand->id, sizeof(xnand->id));

	xnand->clk_rate = xfer->clock  * 2000000;

	read = &xnand->reg.read;

	rdatapin = (xfer->datapin >> 0) & 0xf;
	if (rdatapin == 1) {
		read->op_cfg = LOTUS_FMC100_OP_CFG_MEM_IFTYPE_STD |
			LOTUS_FMC100_OP_CFG_DUMMY_NUM(1);
		read->op_ctrl = LOTUS_FMC100_OP_CTRL_DMA_OP_READY |
			LOTUS_FMC100_OP_CTRL_RD_OPCODE(SPINAND_OP_READ);
	} else if (rdatapin == 2) {
		read->op_cfg = LOTUS_FMC100_OP_CFG_MEM_IFTYPE_DUAL |
			LOTUS_FMC100_OP_CFG_DUMMY_NUM(1);
		read->op_ctrl = LOTUS_FMC100_OP_CTRL_DMA_OP_READY |
			LOTUS_FMC100_OP_CTRL_RD_OPCODE(SPINAND_OP_READ_2);
	} else { /* default rdatapin == 4 */
		read->op_cfg = LOTUS_FMC100_OP_CFG_MEM_IFTYPE_QUAD |
			LOTUS_FMC100_OP_CFG_DUMMY_NUM(1);
		read->op_ctrl = LOTUS_FMC100_OP_CTRL_DMA_OP_READY |
			LOTUS_FMC100_OP_CTRL_RD_OPCODE(SPINAND_OP_READ_4);
	}

	write = &xnand->reg.write;

	wdatapin = (xfer->datapin >> 4) & 0xf;
	if (wdatapin == 1) {
		write->op_cfg = LOTUS_FMC100_OP_CFG_MEM_IFTYPE_STD;
		write->op_ctrl = LOTUS_FMC100_OP_CTRL_DMA_OP_READY |
			LOTUS_FMC100_OP_CTRL_RW_OP_WRITE |
			LOTUS_FMC100_OP_CTRL_WR_OPCODE(SPINAND_OP_PLD);
	} else if (wdatapin == 2) {
		write->op_cfg = LOTUS_FMC100_OP_CFG_MEM_IFTYPE_DUAL;
		write->op_ctrl = LOTUS_FMC100_OP_CTRL_DMA_OP_READY |
			LOTUS_FMC100_OP_CTRL_RW_OP_WRITE |
			LOTUS_FMC100_OP_CTRL_WR_OPCODE(SPINAND_OP_PLD);
	} else { /* default wdatapin == 4 */
		write->op_cfg = LOTUS_FMC100_OP_CFG_MEM_IFTYPE_QUAD;
		write->op_ctrl = LOTUS_FMC100_OP_CTRL_DMA_OP_READY |
			LOTUS_FMC100_OP_CTRL_RW_OP_WRITE |
			LOTUS_FMC100_OP_CTRL_WR_OPCODE(SPINAND_OP_PLD_4);
	}
}
/******************************************************************************/

static int lotus_fmc100_host_set_syncmode(struct lotus_fmc_xnand *xnand,
		struct nand_sync_timing *timing, unsigned int enable)
{
	...
	regval &= ~LOTUS_FMC100_CFG_NFMODE_MASK;
	...
	case NAND_MODE_SYNC_ONFI23:
		regval |= LOTUS_FMC100_CFG_NFMODE_ONFI23;
		break;
	case NAND_MODE_SYNC_ONFI30:
		regval |= LOTUS_FMC100_CFG_NFMODE_ONFI30;
		break;
	case NAND_MODE_SYNC_TOGGLE10:
		regval |= LOTUS_FMC100_CFG_NFMODE_TOGGLE10;
		break;
	case NAND_MODE_SYNC_TOGGLE20:
		regval |= LOTUS_FMC100_CFG_NFMODE_TOGGLE20;
		break;
	default:
		break;
	}
	...
	lotus_fmc_write(host, sync_timming, LOTUS_FMC100_SYNC_TIMING);
...
static int lotus_fmc100_xnand_set_syncmode(struct lotus_fmc_xnand *xnand)
{
	struct lotus_fmc_host *host = xnand->host;
	...
	nand_sync_timing = nand_get_sync_timing(lotus_fmc100_nand_sync_timing,
				xnand->id, sizeof(xnand->id));
	if (!nand_sync_timing)
		return 0;
	...
	lotus_fmc100_set_sync_timing(nand_sync_timing);
	lotus_fmc100_host_set_syncmode(xnand, nand_sync_timing, 1);
...
{
	struct mtd_info *mtd = nand_to_mtd(chip);
	struct lotus_fmc_xnand *xnand = chip->priv;
	...
	ecc->read_page_raw = lotus_fmc100_xnand_read_page;
	ecc->read_page = lotus_fmc100_xnand_read_page;
	ecc->write_page = lotus_fmc100_xnand_write_page;
	ecc->write_page_raw = lotus_fmc100_xnand_write_page;
...
/*****************************************************************************/
static int lotus_fmc100_xnand_probe_device(struct platform_device *pdev,
				       struct lotus_fmc_xnand *xnand)
{
	int ret;
	...
	chip->legacy.cmdfunc = lotus_fmc100_xnand_cmdfunc;
	chip->legacy.dev_ready = lotus_fmc100_xnand_dev_ready;
	chip->legacy.select_chip = lotus_fmc100_xnand_select_chip;
	chip->legacy.read_byte = lotus_fmc100_xnand_read_byte;
	chip->legacy.read_buf = lotus_fmc100_xnand_read_buf;
	chip->legacy.write_buf = lotus_fmc100_xnand_write_buf;
	chip->legacy.IO_ADDR_R = chip->legacy.IO_ADDR_W = xnand->host->iobase;

	if (xnand->ifmode == LOTUS_FMC_IFMODE_NAND) {
		lotus_fmc100_xnand_set_syncmode(xnand);
	...
	ret = nand_scan_with_ids(chip, LOTUS_FMC100_OP_CFG_NUM_CS, nand_id_table);
	if (ret)
		return ret;

	if (xnand->ifmode == LOTUS_FMC_IFMODE_SPINAND) {
		lotus_fmc100_spinand_set_xfer(xnand);
	} else {
	...
	lotus_fmc100_xnand_sanely_check(mtd, chip, xnand);
...
#ifdef CONFIG_LOTUS_NAND_FS_MAY_NO_YAFFS2
	if (xnand->ecc_strength == 16) {
		if (xnand->pagesize == SZ_2K) {
			/* EB bits locate in the bottom two of CTRL(6) */
			xnand->epmoffset = 6;
		} else if (xnand->pagesize == SZ_4K) {
...
#define _SET_FUNC(_func)  \
	...
	mtd->_func = lotus_fmc100_xnand_mtd##_func;
	 ...
	 * mtd->_read = nand_read(); {
	 *     nand_do_read_ops(); {
	 *         chip->cmdfunc(mtd, NAND_CMD_READ0, 0x00, page) = lotus_fmc100_xnand_cmdfunc();
	 *         chip->ecc.read_page = lotus_fmc100_xnand_read_page();
	 *         nand_wait_ready(); {
	 *             chip->dev_ready = lotus_fmc100_xnand_dev_ready();
	 *         }
	 *     }
	 * }
	 */
//	_SET_FUNC(_read);
	 ...
	 *             chip->ecc.write_page = lotus_fmc100_xnand_write_page();
	 *             chip->cmdfunc(mtd, NAND_CMD_PAGEPROG, -1, -1);
	 *             chip->waitfunc(mtd, chip) = nand_wait(); {
	 *                 chip->dev_ready();
	 *                 chip->read_byte() = lotus_fmc100_xnand_read_byte();
	 *             }
	 *         }
	 *     }
	 * }
...
//	_SET_FUNC(_write);
	 ...
	 *             chip->read_buf(mtd, chip->oob_poi, mtd->oobsize) = lotus_fmc100_xnand_read_buf();
	 *         }
	 *         nand_transfer_oob();
	 *         nand_wait_ready();
	 *     }
...
static void lotus_fmc100_spinand_resume(struct lotus_fmc_xnand *xnand)
{
	u32 regval;
	struct lotus_fmc_host *lotus_fmc = xnand->host;
	struct nand_chip *chip = xnand->chip;
	...
	lotus_fmc->set_ifmode(lotus_fmc, LOTUS_FMC_IFMODE_SPINAND);
	clk_set_rate(lotus_fmc->clk, lotus_fmc->spinand->clk_rate);
...
static void lotus_fmc100_nand_suspend(struct lotus_fmc_xnand *xnand)
{
	struct lotus_fmc_host *lotus_fmc = xnand->host;
	...
	lotus_fmc100_host_set_syncmode(xnand, NULL, 0);

	mutex_unlock(&lotus_fmc->lock);
}

static void lotus_fmc100_nand_resume(struct lotus_fmc_xnand *xnand)
{
	u32 regval;
	struct lotus_fmc_host *lotus_fmc = xnand->host;
	struct nand_chip *chip = xnand->chip;
	...
	lotus_fmc_write(lotus_fmc, xnand->reg.fmc_pnd_pwidth_cfg,
		LOTUS_FMC100_PND_PWIDTH_CFG);

	lotus_fmc->set_ifmode(lotus_fmc, LOTUS_FMC_IFMODE_NAND);
	clk_set_rate(lotus_fmc->clk, lotus_fmc->nand->clk_rate);
...
static void lotus_fmc100_spinand_setup(struct lotus_fmc_xnand *xnand)
{
	xnand->regop->read_sr = lotus_fmc100_spinand_read_sr;
	xnand->read_raw_bbm = lotus_fmc100_spinand_read_raw_bbm;
	xnand->cmd_readid = lotus_fmc100_spinand_cmd_readid;
	xnand->cmd_reset = lotus_fmc100_spinand_cmd_reset;
	xnand->cmd_erase = lotus_fmc100_spinand_cmd_erase;

	xnand->resume = lotus_fmc100_spinand_resume;

	xnand->clk_rate = 0; /* min clock do read id, reset.*/
}
/******************************************************************************/

static void lotus_fmc100_nand_setup(struct lotus_fmc_xnand *xnand)
{
	struct lotus_fmc_reg *reg;

	reg = &xnand->reg.read;

	reg->op_cfg = LOTUS_FMC100_OP_CFG_ADDR_NUM(xnand->read_addr_cycle);
	...
	xnand->regop->read_sr = lotus_fmc100_nand_read_sr;
	xnand->read_raw_bbm = lotus_fmc100_nand_read_raw_bbm;
	xnand->cmd_readid = lotus_fmc100_nand_cmd_readid;
	xnand->cmd_reset = lotus_fmc100_nand_cmd_reset;
	xnand->cmd_erase = lotus_fmc100_nand_cmd_erase;

	xnand->resume = lotus_fmc100_nand_resume;
	xnand->suspend = lotus_fmc100_nand_suspend;
			 ...
			 int ifmode)
{
	...
	xnand = devm_kzalloc(&pdev->dev, sizeof(struct lotus_fmc_xnand), GFP_KERNEL);
	if (!xnand) {
		pr_err(DEVNAME"failed to allocate lotus_fmc_xnand structure.\n");
		return -ENOMEM;
	}
	...
	xnand->regop->read_reg = lotus_fmc100_read_reg;
	xnand->regop->write_reg = lotus_fmc100_write_reg;
	xnand->regop->wait_device_ready = lotus_fmc100_xnand_wait_device_ready;

	if (ifmode == LOTUS_FMC_IFMODE_NAND) {
		ctrl_info = lotus_fmc100_nand_ctrl_info;

		host->nand = xnand;
		xnand->name = "nand";

		lotus_fmc100_nand_setup(xnand);
	} else if (ifmode == LOTUS_FMC_IFMODE_SPINAND) {
		ctrl_info = lotus_fmc100_spinand_ctrl_info;

		host->spinand = xnand;
		xnand->name = "spinand";

		lotus_fmc100_spinand_setup(xnand);
	} else
	...
	host->set_ifmode(host, LOTUS_FMC_IFMODE_INVALID);

	lotus_fmc100_xnand_select_chip(xnand->chip, 0);
	xnand->cmd_reset(xnand);
	xnand->cmd_readid(xnand);
	...
	xnand->ecc_strength =
		lotus_fmc100_ecc_strength_reg(host->reg.fmc_cfg_ecc_type, false);
	xnand->pagesize = lotus_fmc100_pagesize_reg(host->reg.fmc_cfg_page_size, false);
	...
	if (lotus_fmc100_xnand_probe_device(pdev, xnand))
		goto fail;

	if (ifmode == LOTUS_FMC_IFMODE_NAND && xnand->manuf != NAND_MFR_AMD)
		read_ahead = lotus_fmc100_nand_read_ahead;
	...
	regop_intf->open    = lotus_fmc100_xnand_regop_open;
	regop_intf->release = lotus_fmc100_xnand_regop_release;
...
int lotus_fmc100_spinand_read_reg(struct nand_chip *chip, u8 reg, u8 *val)
{
	struct lotus_fmc_xnand *xnand = (struct lotus_fmc_xnand *)chip->priv;
	struct flash_regop *regop = xnand->regop;
	struct flash_regop_info info = {0};

	lotus_fmc100_xnand_select_chip(chip, 0);
...
EXPORT_SYMBOL_GPL(lotus_fmc100_spinand_read_reg);

int lotus_fmc100_spinand_wrtie_reg(struct nand_chip *chip, u8 reg, u8 val)
{
	struct lotus_fmc_xnand *xnand = (struct lotus_fmc_xnand *)chip->priv;
	struct flash_regop *regop = xnand->regop;
	struct flash_regop_info info = {0};
...
EXPORT_SYMBOL_GPL(lotus_fmc100_spinand_wrtie_reg);


int lotus_fmc100_spinand_cmd_opt(struct mtd_info *mtd, struct mtd_cmd_opt *cmd_opt)
{
	struct flash_regop_info info = {0};
	...
	info.priv = regop->priv;
	info.cmd = cmd_opt->cmd;
	info.nr_cmd = 1;
	info.addrl = cmd_opt->addr;
	info.addr_cycle = cmd_opt->addr_cycle;
	info.dummy = cmd_opt->dummy;
	info.buf = cmd_opt->data;
	info.sz_buf = cmd_opt->data_len;
...
EXPORT_SYMBOL_GPL(lotus_fmc100_spinand_cmd_opt);
