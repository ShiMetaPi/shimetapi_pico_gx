...
/*
 * The xmedia version of this block adds some bits
 * in SSP_CR1
 */
#define SSP_CR1_MASK_BIGEND_XMEDIA	(0x1UL << 4)
#define SSP_CR1_MASK_ALTASENS_XMEDIA	(0x1UL << 6)

#define SSP_TX_FIFO_CR(r)  (r + 0x28)
#define SSP_RX_FIFO_CR(r)  (r + 0x2C)

#define SSP_FIFOCR_MASK_TXINT_XMEDIA	(0x7UL << 3)
#define SSP_FIFOCR_MASK_RXINT_XMEDIA	(0x7UL << 3)

#define SSP_FIFOCR_MASK_DMATXBR_XMEDIA	(0x7UL << 0)
#define SSP_FIFOCR_MASK_DMARXBR_XMEDIA	(0x7UL << 0)
...
#ifdef CONFIG_ARCH_XMEDIA
#define PL022_IDS_INDEX_XMEDIA		4
#endif
...
static void internal_cs_control(struct pl022 *pl022, u32 command)
{
	...
	if (pl022->vendor->extended_cr && (adev->periphid ==
				adrv->id_table[PL022_IDS_INDEX_XMEDIA].id)) {
		if (pl022->cs_data) {
...
#ifdef CONFIG_ARCH_XMEDIA
/* xmedia versions extend this register to use all 16 bits */
#define DEFAULT_SSP_REG_CR1_XMEDIA ( \
	DEFAULT_SSP_REG_CR1 | \
	GEN_MASK_BITS(SSP_RX_MSB, SSP_CR1_MASK_BIGEND_XMEDIA, 4) | \
	GEN_MASK_BITS(0x0, SSP_CR1_MASK_ALTASENS_XMEDIA, 6) \
)
#endif
...
static void load_ssp_default_config(struct pl022 *pl022)
{
	if (pl022->vendor->pl023) {
		...
		if (adev->periphid == adrv->id_table[PL022_IDS_INDEX_XMEDIA].id) {
			writew(DEFAULT_SSP_REG_CR0, SSP_CR0(pl022->virtbase));
			writew(DEFAULT_SSP_REG_CR1_XMEDIA,
					SSP_CR1(pl022->virtbase));
		} else {
#endif
...
static int pl022_setup(struct spi_device *spi)
{
	...
	if (pl022->vendor->extended_cr) {
		...
		SSP_WRITE_BITS(chip->cr1, tmp, SSP_CR1_MASK_BIGEND_XMEDIA, 4);
		SSP_WRITE_BITS(chip->cr1, 0x0, SSP_CR1_MASK_ALTASENS_XMEDIA, 6);
#endif
	} else {
		SSP_WRITE_BITS(chip->cr0, bits - 1,
			       SSP_CR0_MASK_DSS, 0);
