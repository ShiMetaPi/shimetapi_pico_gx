...
#if defined(CONFIG_LOTUS_FMC100)
extern int lotus_fmc100_cmd_opt(struct mtd_info *mtd, struct mtd_cmd_opt *cmd_opt);
#endif
...
static int mtdchar_ioctl(struct file *file, u_int cmd, u_long arg)
{
	...
	case MTDCMDOPT:
	{
		struct mtd_cmd_opt cmd_opt;

		memset(&cmd_opt, 0, sizeof(cmd_opt));
		ret = copy_from_user(&cmd_opt, argp, sizeof(cmd_opt));
		if (ret)
			ret = -EFAULT;

		ret = lotus_fmc100_cmd_opt(master, &cmd_opt);
		if (ret)
			ret = -EFAULT;

		if (copy_to_user(argp, &cmd_opt, sizeof(cmd_opt)))
			return -EFAULT;
