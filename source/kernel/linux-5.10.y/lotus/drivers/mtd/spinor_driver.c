...
extern struct spinor_flash_dev spinor_flash_ids_lotus[];
...
static struct spinor_driver spinor_drv_spansion_4byte = {
	.quad_enable = spinor_quad_enable_spansion,
	.quad_disable = spinor_quad_disable_spansion,
	.entry_4addr = spinor_entry_4addr_4byte,
	.exit_4addr = spinor_exit_4addr_4byte,
...
static int spinor_quad_enable_normem(struct flash_regop *regop)
{
	u8 sr[2];

	spinor_read_buf_reg(regop, SPINOR_OP_RDSR2, &sr[1], 1);
	if (sr[1] & SPINOR_STATUS2_EQ_NM)
		return 0;

	spinor_read_buf_reg(regop, SPINOR_OP_RDSR,  &sr[0], 1);

	sr[1] |= SPINOR_STATUS2_EQ_NM;

	spinor_write_enable(regop);

	spinor_write_buf_reg(regop, SPINOR_OP_WRSR, &sr[0], 1);

	spinor_write_enable(regop);

	spinor_write_buf_reg(regop, SPINOR_OP_WRSR, &sr[1], 1);
...
static int spinor_quad_disable_normem(struct flash_regop *regop)
{
	u8 sr[2];
...
static struct spinor_driver spinor_drv_normem  = {
	.quad_enable = spinor_quad_enable_normem,
	.quad_disable = spinor_quad_disable_normem,
};
...
static struct spinor_drvier_table spinor_drv_table[] = {
	...
	/* GigaDevice */
	{{ 0xC8, 0x40, 0x19 }, 3, &spinor_drv_spansion_4byte },
	/* GigaDevice */
	{{ 0xC8, 0x40 }, 2, &spinor_drv_spansion },
	/* Winbond */
	{{ 0xef, 0x40, 0x19 }, 3, &spinor_drv_spansion_4byte },
	/* Winbond */
	{{ 0xef, 0x40 }, 2, &spinor_drv_spansion },
	/* Zbit */
	{{ 0x5e, 0x40 }, 2, &spinor_drv_spansion },
	/* FM 3.3V */
	{{ 0xa1, 0x40 }, 2, &spinor_drv_spansion },
	/* CFX 3.3V */
	{{ 0x1c, 0x40 }, 2, &spinor_drv_spansion },
	/* NOR MEM 3.3V */
	{{ 0x52, 0x22 }, 2, &spinor_drv_normem },
	{{ 0x52, 0x21 }, 2, &spinor_drv_normem },
	/* XTX 3.3v */
	{{ 0x0B, 0x40 }, 2, &spinor_drv_spansion },
	/* Puya 3.3V */
	{{ 0x85, 0x60 }, 2, &spinor_drv_spansion },
	{{ 0x85, 0x20 }, 2, &spinor_drv_spansion },
	/* EON 3.3V */
	{{ 0x1c, 0x71 }, 2, &spinor_drv_spansion },
	{{0}, 0, 0},
};
...
struct spinor_flash_dev *spinor_match_id(u8 *id, int id_len)
{
	...
	for (spinor = spinor_flash_ids_lotus; spinor->name; spinor++) {
