...
#if defined(CONFIG_MTD_SPI_NOR) && defined(CONFIG_SPI_XMEDIA_SFC)
static int get_mtd_device_info(struct mtd_info *mtd, struct mtd_device_info *info)
{
	int ret;


	if (mtd->type != MTD_NORFLASH) {
		pr_err("Just spi nor flash could be get the device info Now.\n");
		return -EINVAL;
	}

	ret = spi_nor_read_dev_info(mtd, info->id, info->id_len, info->cmd, info->addr, info->addr_len, info->dummy_len);
	if (ret < 0)
		return -EFAULT;
...
static int mtdchar_ioctl(struct file *file, u_int cmd, u_long arg)
{
	...
	case MTDDEVICEID:
	{
		...
		ret = get_mtd_device_info(mtd, &info);
		if (ret)
			return -EFAULT;
