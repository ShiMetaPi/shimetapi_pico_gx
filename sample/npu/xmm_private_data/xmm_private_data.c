...
int main()
{
    ...
    xmedia_cl_mem_info model_info;
    xmedia_cl_model_info_type info_type = XMEDIA_CL_MEM_INFO;
    //公共模块初始化
    ...
    ret = xmedia_cl_graph_query_model_info_from_file(model, (void *)&model_info,
                                                     info_type);
    if (ret != XMEDIA_CL_SUCCESS) {
        printf("xmedia_cl_graph_query_model_info_from_file err, errno %d\n",
               ret);
        goto ERROR;
    }
    ...
    data_size = model_info.private_data_size;
    private_data = (xmedia_cl_s32 *)malloc(data_size);
    if (private_data == NULL) {
        printf("private data malloc error\n");
        goto ERROR;
    }
    memset(private_data, 0, data_size);
    ret = xmedia_cl_graph_get_private_data_from_file(
        model, (void *)private_data, data_size);
    if (ret != XMEDIA_CL_SUCCESS) {
        printf("xmedia_cl_graph_get_private_data_from_file err, errno %d\n",
               ret);
        goto ERROR;
    }
