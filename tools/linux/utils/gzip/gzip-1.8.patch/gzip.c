    ...
    char *iname;
{
    ...
    /* Add hw gzip header */
    if (!decompress && !strcmp(program_name, "hwgzip")) {
        unsigned int out_size = 0;
        const unsigned int magic_num0 = 0x70697A67; /* HW gzip header magic num0 */
        const unsigned int magic_num1 = 0x64616568; /* HW gzip header magic num0 */
        ssize_t count = 0;
        fprintf(stderr, "%s: Add hw gzip header to %s\n", program_name, ofname);
        count  = write(ofd, &out_size, 4);
        count += write(ofd, &ifile_size, 4);
        count += write(ofd, &magic_num0, 4);
        count += write(ofd, &magic_num1, 4);
        if (count != 16) { /* HW gzip header size is 16 bytes */
            fprintf(stderr, "%s: fail to write hw gzip header to %s\n", program_name, ofname);
            write_error();
            return;
        }
    }
    ...
    /* Write the commpression size to hw gzip header  */
    if (!decompress && !strcmp(program_name,"hwgzip")) {
        ssize_t count = 0;
        off_t cur = lseek(ofd, 0, SEEK_CUR);
        lseek(ofd, 0, SEEK_SET);
        fprintf(stderr, "%s: Write the compression size to hw gzip header\n", program_name);
        count = write(ofd, &bytes_out, 4);
        lseek(ofd, cur, SEEK_SET);
        if (count != 4) {
            fprintf(stderr, "%s: fail to write compression size to the header of %s\n", program_name, ofname);
            write_error();
            return;
        }
    }
