...
int main(int argc, char **argv)
{
	...
	struct oob_info *info = NULL;
	struct oobuse_info *oobuseinfo = NULL;
	unsigned int pagetype, ecctype, yaffs;

	/* for nandcv610/nandcv620/fmcv100 province pin mode */
	struct reverse_pagesize_ecctype_sector *reverse_page_ecc_sector = NULL;
	struct reverse_blocksize_sector *reverse_block_sector = NULL;
	...
	unsigned int infilesize, pagesize, pageindex = 0, pagecount, random, pagenum, savepin;
	...
	while(pagecount--)
	{
		...
		ret = page_ecc_gen(pagebuf, pagetype, ecctype);
		if (ret < 0)
		{
