...
int sdhci_set_clock(struct mmc *mmc, unsigned int clock)
{
	...
	if (host->type != MMC_TYPE_MMC) {//only sd card
		clk_ctl = sdhci_readw(host, SDHCI_CLOCK_CONTROL);
		clk_ctl &= ~SDHCI_CLOCK_CARD_EN;
		sdhci_writew(host, clk_ctl, SDHCI_CLOCK_CONTROL);
	} else {
...
static void sdhci_set_power(struct mmc *mmc, unsigned short power)
{
	struct sdhci_host *host = mmc->priv;
#else
static void sdhci_set_power(sdhci_host *host, unsigned short power)
{
#endif
	u8 pwr = 0;
...
#ifdef CONFIG_LOTUS_SDHCI
	sdhci_set_power(mmc, fls(mmc->cfg->voltages) - 1);
#else
...
#ifdef CONFIG_LOTUS_SDHCI
static int sdhci_wait_dat0(struct mmc *mmc, int state,
			   int timeout_us)
{
	int tmp;
	struct sdhci_host *host = mmc->priv;
