...
static int start_read_ahead_thread(struct cache_ctrl *ctrl)
{
	...
	if (IS_ERR(task)) {
		pr_err("Failed to create the %s read ahead thread\n", ctrl->name);
		ctrl->read_ahead_runing = false;
		return IS_ERR(task);
	}
...
static int _get_status_lock_read_page(struct flash_cache *cache, u32 pageindex)
{
	...
	if (!flash_cache_try_enable(ctrl))
		return FLASHCACHE_DISABLE;
...
static int resolve_input_bufdeal(struct key_val_t *kv, char **buf)
{
	ulong val;

	if (kv->def_val == 1) {
		val = kv->def_val;
		memcpy(kv->buf, &val, min_t(ulong, kv->sz_buf, sizeof(val)));
	} else if (kv->def_val == 2) { /* 2 : value */
		char *ptr = kv->buf;
...
{
	int ix;
	char buf_enabe[4] = {0}; /* 4 : buf's size */
	...
	while (buf < ptail) {
		...
		for (ix = 0; ix < ARRAY_SIZE(key_val); ix++) {
			...
			if (resolve_input_bufdeal(kv, &buf) != 0)
				return;
...
static const struct proc_ops flash_cache_proc_fops = {
	.proc_read      = proc_fops_read,
	.proc_write     = proc_fops_write,
};

#else

static const struct file_operations flash_cache_proc_fops = {
	.owner = THIS_MODULE,
	.read      = proc_fops_read,
...
static void check_left_time(struct cache_ctrl *ctrl)
{
	...
	if (ctrl->lifetime) {
		...
		schedule_delayed_work(&ctrl->time_work,
			msecs_to_jiffies(ctrl->lifetime * 1000)); /* 1000 */
...
static void set_ctrl_value(struct cache_ctrl *ctrl)
{
...
struct flash_cache *flash_cache_create(struct flash_cache_create_arg *arg)
{
	...
	strncpy(ctrl->name, arg->name, sizeof(ctrl->name) - 1);

	ctrl->pages_per_block = arg->blocksize / arg->pagesize;
	ctrl->pagesize = arg->pagesize;
	ctrl->oobsize = arg->oobsize;
	ctrl->max_pages = (u32)(arg->totalsize >> (unsigned int)(ffs(arg->pagesize) - 1));
	ctrl->max_caches = (arg->nr_cache <= 0 ? ctrl->max_pages : arg->nr_cache);

	ctrl->sz_cache_node = sizeof(struct cache_node) + ctrl->pagesize + ctrl->oobsize;

	if (flash_cache_read_ahead_enable) {
		ctrl->read_ahead = arg->read_ahead;
		ctrl->read_ahead_args = arg->read_ahead_args;
		ctrl->read_ahead_runing = false;
	}

	ctrl->rbcache = kmem_cache_create("flashcache", sizeof(struct cache_node), 0, 0, NULL);
	if (ctrl->rbcache == NULL) {
		kfree(ctrl);
		pr_err("failed to allocate memory.\n");
		return NULL;
	}

	ctrl->dentry = proc_create_data("nandcache", 0644, NULL, &flash_cache_proc_fops, ctrl); /* 0644 */
	if (ctrl->dentry == NULL) {
	...
	ctrl->cache.oobsize = arg->oobsize;

	set_ctrl_value(ctrl);

	check_left_time(ctrl);
