...
#ifdef CONFIG_LOTUS_SDHCI
		if (mmc->cfg->ops->set_power)
			mmc->cfg->ops->set_power(mmc, fls(mmc->cfg->voltages) - 1);
#endif
	return 0;
}
...
#ifdef CONFIG_LOTUS_SDHCI
		/*must set_power befor close clk*/
		if (mmc->cfg->ops->set_power)
			mmc->cfg->ops->set_power(mmc, 0);
#endif
	return 0;
}
...
retry:
	...
	if (err && uhs_en) {
#ifdef CONFIG_LOTUS_SDHCI
		/* only host is SD and switch voltage fail to retries */
		if ((host->type == MMC_TYPE_SD) && (err == -EAGAIN))
			retries--;
		else
			uhs_en = false;
#else
