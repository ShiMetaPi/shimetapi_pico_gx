...
static const struct spi_nor_basic_flash_parameter nm_params = {
	.rd_modes		= SNOR_RD_MODES,
	.reads[SNOR_MIDX_SLOW]	= SNOR_OP_READ(0, 0, SPINOR_OP_READ),
	.reads[SNOR_MIDX_1_1_1]	= SNOR_OP_READ(0, 8, SPINOR_OP_READ_FAST),
	.reads[SNOR_MIDX_1_1_2]	= SNOR_OP_READ(0, 8, SPINOR_OP_READ_1_1_2),
 ...
 *		Winbond         W25Q512nWIM        	64M		64K     1V8
...
static const struct flash_info spi_nor_ids[] = {
	...
	{ "en25qh128a", INFO(0x1c7018, 0, 64 * 1024,  256,
			SPI_NOR_QUAD_READ), PARAMS(eon), CLK_MHZ_2X(80) },
	...
	{ "nm25q128evb", INFO(0x522118,  0, 64 * 1024,  256,
		SPI_NOR_QUAD_READ), PARAMS(nm), CLK_MHZ_2X(50)},
	...
	{ "w25q512nw-im", INFO(0xef8020, 0, 64 * 1024, 1024,
			SECT_4K | SPI_NOR_DUAL_READ | SPI_NOR_QUAD_READ |
			SPI_NOR_HAS_LOCK | SPI_NOR_HAS_TB | SPI_NOR_4B_OPCODES),
			PARAMS(winbond), CLK_MHZ_2X(133) },
	...
	{"p25q128h", INFO(0x856018, 0, 64 * 1024, 256,
		SPI_NOR_QUAD_READ), PARAMS(puya), CLK_MHZ_2X(80) },
	{"p25q64sh", INFO(0x856017, 0, 64 * 1024, 128,
		SPI_NOR_QUAD_READ), PARAMS(puya), CLK_MHZ_2X(80) },
	{ "py25q256hb", INFO(0x852019, 0, 64 * 1024, 512,
		SPI_NOR_QUAD_READ | SPI_NOR_4B_OPCODES), PARAMS(spansion), CLK_MHZ_2X(133) },
	...
	{ "GM25Q64ASIG",  INFO(0x1c4017, 0, 64 * 1024, 128,
		SPI_NOR_QUAD_READ), PARAMS(spansion), CLK_MHZ_2X(80) },
	{ "GM25Q128ASIG", INFO(0x1C4018, 0, 64 * 1024, 256,
		SPI_NOR_QUAD_READ), PARAMS(spansion), CLK_MHZ_2X(80) },
...
static int nm_quad_enable(struct spi_nor *nor)
{
	...
	ret = read_cr(nor);
	if (ret & CR_QUAD_EN_NM)
		return 0;

	ret = read_cr(nor);
	val = ((unsigned int)ret & 0xff) | CR_QUAD_EN_NM;
	...
	/* read back and check it */
	ret = read_cr(nor);
	if (!(ret > 0 && ((unsigned int)ret & CR_QUAD_EN_NM))) {
		dev_err(nor->dev, "Nor-mem Quad bit not set\n");
		return -EINVAL;
	}
...
#endif/* CONFIG_XMEDIA_SPI_BLOCK_PROTECT */
...
#if defined(CONFIG_MTD_SPI_NOR) && defined(CONFIG_SPI_XMEDIA_SFC)
#include "xmedia-sfc.h"
/*
 * Note: struct mtd_part and mtd_to_part  copied from drivers/mtd/mtdpart.c.
 * It may be changed With different linux kernel version.
 */
struct mtd_part {
	struct mtd_info mtd;
...
int spi_nor_read_dev_info(struct mtd_info *mtd, u8 *id, int len, u8 cmd, u32 addr, u8 addr_len, u8 dummy_len)
{
	...
	if ((id == NULL) || len > SPI_ID_LEN_MAX) {
		dev_err(nor->dev, "paramters are not correct, please check.\n");
		return -EINVAL;
	}
	...
	slave = mtd_to_part(mtd);
	master = slave->master;

	nor = mtd_to_spi_nor(master);
	ret = spi_nor_lock_and_prep(nor, SPI_NOR_OPS_READ);
	if (ret)
		return ret;

	ret = bsp_spi_nor_read_reg_with_dummy(nor, cmd, id, len, addr, addr_len, dummy_len);
	if (ret < 0) {
		dev_err(nor->dev, "error %d  read reg failed\n", ret);
	}
	spi_nor_unlock_and_unprep(nor, SPI_NOR_OPS_READ);

	return ret;
}
EXPORT_SYMBOL_GPL(spi_nor_read_dev_info);
#endif
