			  ...
			  struct req_info *req_info)
{
	...
	if (req_info != NULL) {
		...
		if (req_info->data_bittiming != NULL) {
			addattr_l(&req.n, 1024, IFLA_CAN_DATA_BITTIMING,
				  req_info->data_bittiming,
				  sizeof(struct can_bittiming));
		}
...
int can_set_data_bittiming(const char *name, struct can_bittiming *bt, struct can_bittiming *dbt)
{
	struct req_info req_info;
	struct can_ctrlmode cm;
	memset(&req_info, 0, sizeof(req_info));
...
int can_set_data_bitrate(const char *name, __u32 nominal_bitrate, __u32 data_bitrate)
{
	...
	bt.bitrate = nominal_bitrate;
	dbt.bitrate = data_bitrate;

	return can_set_data_bittiming(name, &bt, &dbt);
}
