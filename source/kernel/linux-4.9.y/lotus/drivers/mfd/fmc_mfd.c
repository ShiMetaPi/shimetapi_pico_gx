...
/* ------------------------------------------------------------------------ */
static const struct mfd_cell lotus_fmc_devs[] = {
	{
		.name = "lotus_spi_nor",
		.of_compatible = "lotus,fmc-spi-nor",
	},
	{
		.name = "lotus_spi_nand",
		.of_compatible = "lotus,fmc-spi-nand",
	},
};

static int lotus_fmc_probe(struct platform_device *pdev)
{
	struct lotus_fmc *fmc;
	struct resource *res;
...
#ifdef CONFIG_LOTUS_FMC100
	fmc->sz_iobase = res->end - res->start + 1;

	fmc->irq = platform_get_irq(pdev, 0);
	if (fmc->irq < 0) {
	...
	ret = mfd_add_devices(dev, 0, lotus_fmc_devs,
			      ARRAY_SIZE(lotus_fmc_devs), NULL, 0, NULL);
	if (ret) {
...
static int lotus_fmc_remove(struct platform_device *pdev)
{
	struct lotus_fmc *fmc = platform_get_drvdata(pdev);
...
static const struct of_device_id lotus_fmc_of_match_tbl[] = {
	{ .compatible = "lotus,fmc"},
	{ }
};
MODULE_DEVICE_TABLE(of, lotus_fmc_of_match_tbl);

static struct platform_driver lotus_fmc_driver = {
	.driver = {
		.name = "fmc",
		.of_match_table = lotus_fmc_of_match_tbl,
	},
	.probe = lotus_fmc_probe,
	.remove = lotus_fmc_remove,
};
module_platform_driver(lotus_fmc_driver);

MODULE_LICENSE("GPL v2");
MODULE_DESCRIPTION("Lotus Flash Memory Controller Driver");
