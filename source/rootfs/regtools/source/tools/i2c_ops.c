...
int i2c_read(int argc , char* argv[])
{
	...
	unsigned int i2c_num = 0, dev_addr = 0, reg_addr = 0, reg_addr_end = 0;
	...
	memset(buf, 0x0, I2C_READ_BUF_LEN);
...
int i2c_write(int argc , char* argv[])
{
	...
	char file_name[I2C_NAME_SIZE];
	unsigned char buf[4];
	unsigned int i2c_num = 0, dev_addr = 0, reg_addr = 0, data = 0;
	...
	printf("i2c_num:0x%x, dev_addr:0x%x; reg_addr:0x%x; data:0x%x; reg_width: %u; data_width: %u.\n",
