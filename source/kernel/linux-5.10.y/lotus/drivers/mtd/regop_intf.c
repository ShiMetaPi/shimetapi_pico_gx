...
static int regop_resolve_valset(int ix, struct flash_regop_info *info,
	char *buf, char **retbuf, struct cmd_val_t *val)
{
	ulong value;
	if (ix == 2) { /* 2 : address */
...
static void regop_resolve_bufcheck(char *buf, char **retbuf, char *ptail)
{
	char chr;
	char *nextbuf = buf;
	...
	*op = 0;
	...
	while (buf < ptail) {
		...
		for (ix = 0; ix < ARRAY_SIZE(cmd_val); ix++) {
			...
			if (regop_resolve_valset(ix, info, buf, &buf, val) != 0)
				return NULL;
		...
		if (ix == ARRAY_SIZE(cmd_val))
			regop_resolve_bufcheck(buf, &buf, ptail);
	}
out:
	*msecs = _msecs;
