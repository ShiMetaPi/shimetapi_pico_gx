...
#include "sdhci-xmfalcon-drive-cap.c"
...
#define REG_CTRL_SDIO1_CLK		0x0018
#define REG_CTRL_SDIO1_CMD		0x001C
#define REG_CTRL_SDIO1_DATA0		0x0020
#define REG_CTRL_SDIO1_DATA1		0x0024
#define REG_CTRL_SDIO1_DATA2		0x0028
#define REG_CTRL_SDIO1_DATA3		0x002c
...
#define REG_MISC_CTRL78		0x8808
#define IO_CTRL_MS		1
#define LSADC_DETECT	0

#define REG_MISC_CTRL80		0x8810
#define IO_FLASH_MS		0x10
#define POC_FLASH_MS	1
...
static void lotus_set_drv_str(struct regmap *iocfg_regmap,
...
static void lotus_set_mmc_drv(struct sdhci_host *host)
{
	...
	int bus_width = host->mmc->ios.bus_width == MMC_BUS_WIDTH_8 ? 8 : 4;
	int i;
	...
	const int emmc_speed[] = {
		MMC_TIMING_MMC_HS400,  4,
		MMC_TIMING_MMC_HS200,  3,
		MMC_TIMING_MMC_DDR52,  2,
		MMC_TIMING_MMC_HS,     1,
		MMC_TIMING_LEGACY,     0,
	};
	...
	regmap_read(sysctrl_regmap, BSP_CFG_CRG_REG, &emmc_intf);
	...
	pr_debug("%s: set_drv:%d %d %d(%d)\n", mmc_hostname(host->mmc),
			layer_index, inch_index, speed_index, emmc_drive_offset);

	if (bus_width == 8) {
		regmap_read(sysctrl_regmap, REG_MISC_CTRL80, &reg);
		regmap_read(sysctrl_regmap, REG_MISC_CTRL78, &_reg);
		if ((_reg & IO_CTRL_MS) == LSADC_DETECT) {
			if (reg & IO_FLASH_MS) {
				/* flash电源域供电1.8V, 需切换SDIO0电源域供电电压为1.8V */
				/* step1: 配置POWER SWITCH输出电压为1.8V */
				regmap_read(sysctrl_regmap, REG_MISC_CTRL71, &reg);
				reg |= PS_POWER_SEL;
				regmap_write(sysctrl_regmap, REG_MISC_CTRL71, reg);
	...
	lotus_set_drv_str(iocfg_regmap,
		REG_CTRL_EMMC_CLK, 0, 1,
		emmc_drive->clk_pad & 0x01,
		emmc_drive->clk_pad >> 1); /* set drv level */

	lotus_set_drv_str(iocfg_regmap,
		REG_CTRL_EMMC_CMD, 1, 0,
		emmc_drive->dq_cmd_pad & 0x01,
		emmc_drive->dq_cmd_pad >> 1); /* set drv level */

	for (i = 0; i < bus_width; i++)
		lotus_set_drv_str(iocfg_regmap,
			io_emmc_data_reg[i], 1, 0,
			emmc_drive->dq_cmd_pad & 0x01,
			emmc_drive->dq_cmd_pad >> 1); /* set drv level */

	lotus_set_drv_str(iocfg_regmap,
		REG_CTRL_EMMC_RST, 1, 0, 1, 0x0); /* set drv level */

	if (host->timing == MMC_TIMING_MMC_HS400) {
		lotus_set_drv_str(iocfg_regmap,
			REG_CTRL_EMMC_DS, 1, 0,
			emmc_drive->dq_cmd_pad & 0x01,
			emmc_drive->dq_cmd_pad >> 1); /* set drv level */
	}
...
static void lotus_set_sd_drv(struct sdhci_host *host)
{
	struct sdhci_lotus_priv *lotus_priv = sdhci_get_pltfm_priv(host);
	void *iocfg_regmap = lotus_priv->iocfg_regmap;
	int i;
	...
	const int sdio_speed[] = {
		...
		MMC_TIMING_UHS_DDR50,  3,
		MMC_TIMING_UHS_SDR50,  2,
		MMC_TIMING_UHS_SDR25,  1,
		MMC_TIMING_UHS_SDR12,  1,
		MMC_TIMING_SD_HS,      0,
		MMC_TIMING_LEGACY,     0,
	};
	...
	regmap_read(sysctrl_regmap, BSP_CFG_CRG_REG, &sdio_intf);
...
static void lotus_set_sd_voltage(struct sdhci_host *host, unsigned char signal_voltage)
{
	struct sdhci_lotus_priv *lotus_priv = sdhci_get_pltfm_priv(host);
	void *sysctrl_regmap = lotus_priv->sysctrl_regmap;
...
static void lotus_set_sdio_drv(struct sdhci_host *host)
{
	struct sdhci_lotus_priv *lotus_priv = sdhci_get_pltfm_priv(host);
	...
	unsigned int sdio_intf, sdio_drive_offset;
	unsigned int inch_index, layer_index, speed_index;
	struct sdhci_pad_cell *sdio_drive = sdio1_drive_cap;
	...
	/* bit[5-6]: layer	bit[11-12]: inch*/
	regmap_read(sysctrl_regmap, BSP_CFG_CRG_REG, &sdio_intf);
	layer_index = (sdio_intf >> 5) & 0x03;
	inch_index = (sdio_intf >> 12) & 0x03;
	...
	lotus_set_drv_str(iocfg3_regmap,
		REG_CTRL_SDIO1_CLK, 0, 1,
		sdio_drive->clk_pad & 0x01,
		sdio_drive->clk_pad >> 1); /* set drv level */

	lotus_set_drv_str(iocfg3_regmap,
		REG_CTRL_SDIO1_CMD, 1, 0,
		sdio_drive->dq_cmd_pad & 0x01,
		sdio_drive->dq_cmd_pad >> 1); /* set drv level */

	for (i = 0; i < IO_CFG_SDIO1_DATA_LINE_COUNT; i++)
		lotus_set_drv_str(iocfg3_regmap,
			io_sdio1_data_reg[i], 1, 0,
			sdio_drive->dq_cmd_pad & 0x01,
			sdio_drive->dq_cmd_pad >> 1); /* set drv level */
...
static void lotus_set_io_config(struct sdhci_host *host)
{
	...
	if (devid == 0) {
		...
		/* For mmc1: SD card */
		lotus_set_sd_drv(host);
	} else {
		/* For mmc2: sdio wifi */
		lotus_set_sdio_drv(host);
	}
...
static void lotus_set_io_voltage(struct sdhci_host *host, unsigned char signal_voltage)
{
	...
	if (lotus_priv->devid == 1) {
		/* For mmc1: SD card */
		lotus_set_sd_voltage(host, signal_voltage);
	}
}
