	...
	if (cmd->flags & MMC_RSP_PRESENT) {
		...
		if (((cmd->flags & MMC_RSP_R1) == MMC_RSP_R1) &&
			...
			if ((cmd->resp[0] & CMD_ERRORS) && !host->is_tuning) {
				...
				pr_err("%s: The status of the card is abnormal, cmd->resp[0]: %x",
...
#ifdef CONFIG_ARCH_LOTUS
			host->ops->set_io_voltage(host, ios->signal_voltage);
#endif
			return 0;
		}
	...
	do {
		...
		if (intmask & (SDHCI_INT_CARD_INSERT | SDHCI_INT_CARD_REMOVE)) {
			...
			host->thread_isr |= intmask & (SDHCI_INT_CARD_INSERT |
						       SDHCI_INT_CARD_REMOVE);
#ifdef CONFIG_ARCH_LOTUS
			if (host->thread_isr & SDHCI_INT_CARD_REMOVE)
				sdhci_writeb(host, 0, SDHCI_POWER_CONTROL);
#endif
