...
static inline int image_check_arch(const image_header_t *hdr, uint8_t arch)
{
	...
	/* For lotus, we need to support warm-reseting from aarch32 state
	 * to aarch64 uImage, so the image can be an aarch64 uImage even though
	 * the arch in u-boot is aarch32.
	 */
