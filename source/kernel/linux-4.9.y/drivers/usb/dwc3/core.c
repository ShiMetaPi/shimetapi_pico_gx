...
static char setup_zones[MAX_DR_STRING_LEN] = {'\0'};
module_param_string(dr_mode, setup_zones, MAX_DR_STRING_LEN, 0600);
...
enum usb_dr_mode get_dr_mode_from_param(void)
{
	...
	if (setup_zones[0] != 0) {
		...
		if (strncmp(setup_zones, "host", MAX_DR_STRING_LEN) == 0) {
			usb_dr_mode = USB_DR_MODE_HOST;
			printk("module dr type: host\n");
		} else if (strncmp(setup_zones, "peripheral", MAX_DR_STRING_LEN) == 0) {
