...
#ifdef CONFIG_ARCH_LOTUS
	bool hwzimage_flag = check_hwzimage_header(image_buf);
	if (hwzimage_flag) {
		image_buf += HW_GZIP_HEAD_SIZE;
		image_len -= HW_GZIP_HEAD_SIZE;
	}
#endif
...
#ifdef CONFIG_GZIP
	case IH_COMP_GZIP: {
#if defined(CONFIG_ARCH_LOTUS) && defined(CONFIG_HWDEC) && defined(CONFIG_CMD_HW_GUNZIP)
		if (hwzimage_flag)
			ret = hw_gunzip(load_buf, unc_len, image_buf, (uint *)&image_len);
                else
#endif
