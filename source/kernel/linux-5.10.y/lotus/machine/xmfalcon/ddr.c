...
u32 ddrphy_readl(u32 reg)
{
	WARN_ON(!ddrphy_base);

	return readl(ddrphy_base + reg);
}

void ddrphy_writel(u32 v, u32 reg)
{
	WARN_ON(!ddrphy_base);

	writel(v, ddrphy_base + reg);
}
...
int ddrc_init(void)
{
	if (ddrc_base == NULL) {
		...
		ddrphy_base = ioremap(REG_DDRPHY_BASE, 64 * 1024);
		if (ddrphy_base == NULL) {
			iounmap(ddrc_base);
			pr_err("ddrphy map failed\n");
			return -ENOMEM;
		}
	}
