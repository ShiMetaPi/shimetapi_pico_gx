		  ...
		  struct net_device *dev)
{
	...
	/*by zhoujiang fix drop NETIF_F_UFO
...
static int femac_fill_sg_desc(const struct femac_priv *priv,
		  struct sk_buff *skb, unsigned int pos)//by zhoujiang fix
{
	struct tx_desc *desc_cur;
	dma_addr_t addr;
	int ret;
	...
	desc_cur->ipv6_id = ipv6_proxy_select_ident(dev_net(skb->dev), skb);//by zhoujiang fix
	...
	for (i = 0; i < skb_shinfo(skb)->nr_frags; i++) {
		...
		int len = skb_frag_size(frag);//int len = frag->size; //by zhoujiang fix to function
...
{
	//by zhoujiang fix dma alloc and clear function(dma_zalloc_coherent)
	priv->tx_ring.desc = (struct tx_desc *)dma_alloc_coherent(priv->dev,
		TXQ_NUM * sizeof(struct tx_desc), &priv->tx_ring.dma_phys, GFP_KERNEL | __GFP_ZERO);
...
//by zhoujiang add
/**
 * femac_change_mtu - set new MTU size
 * @dev: network device
 ...
 */
int femac_change_mtu(struct net_device *dev, int new_mtu)
{
...
static const struct net_device_ops femac_netdev_ops = {
	...
	.ndo_change_mtu = femac_change_mtu,//by zhoujiang fix(eth_change_mtu)
		  ...
		  struct femac_priv *priv)
{
	...
	if (of_device_is_compatible(node, "lotus,femac-v2"))
		  ...
		  struct femac_priv *priv)
{
	...
	if (!IS_ERR(mac_addr))//by zhoujiang fix
		  ...
		  struct femac_priv *priv, int *ret)
{
	...
	if (phy == NULL) {
		...
		if (of_phy_is_fixed_link(node)) {
			*ret = of_phy_register_fixed_link(node);
			 ...
			 */
			if(of_get_phy_mode(node, &interface)){
				dev_err(dev, "get interface failed!\n");
				*ret = -ENODEV;
				return phy;
			}

			phy = of_phy_connect(priv->ndev, of_node_get(node), &femac_adjust_link, 0, interface);
			if (phy == NULL) {
	...
	linkmode_clear_bit(ETHTOOL_LINK_MODE_1000baseT_Full_BIT, phy->advertising);
	linkmode_clear_bit(ETHTOOL_LINK_MODE_1000baseT_Half_BIT, phy->advertising);
...
MODULE_DESCRIPTION("Lotus Fast Ethernet MAC driver");
MODULE_AUTHOR("Lotus");
