...
#define REG_FW_STATE		0x30e2

#define PHY_RESET_DELAYS_PROPERTY	"xmedia,phy-reset-delays-us"
...
/* This dummy function is just for creating dependency in modules.dep with the femac
 * driver when installing modules, and making modprobe to load this phy driver before
 * loading the femac driver.*/
void femac_mdio_dummy(void){}
EXPORT_SYMBOL(femac_mdio_dummy);

struct femac_mdio_data *g_mdio_data = NULL;
struct xmedia_fephy_private fephy_private;
				      ...
				      u32 reg_addr)
{
	...
	if (fephy_private.fephy_init_state)
...
static unsigned int fephy_get_firmware_state(void)
{
	struct mii_bus *bus = g_mdio_data->bus;
	u32 phy_addr = g_mdio_data->phy_addr;

	return fephy_expanded_read(bus, phy_addr, REG_FW_STATE);
}

static void fephy_mask_firmware_state(void)
{
	struct mii_bus *bus = g_mdio_data->bus;
	u32 phy_addr = g_mdio_data->phy_addr;

	fephy_expanded_write(bus, phy_addr, REG_FW_STATE, 0);
}

static void fephy_reset(void)
{
	femac_fephy_reset_and_trim(g_mdio_data);
}

static void opt_method0(void)
{
	void __iomem *base = NULL;

	base = ioremap(IO0_BASE, PAGE_SIZE);
	if (base == NULL) {
		pr_err("ioremap failed\n");
	}
	writel(LEVEL0, base + IO0_CFG1);
	writel(IO0_CFG, base + IO0_CTRL);
	msleep(100);
	writel(LEVEL1, base + IO0_CFG1);
	writel(IO0_CFG, base + IO0_CTRL);
	msleep(20);
...
static void opt_method1(void)
{
	void __iomem *base = NULL;

	base = ioremap(IO1_BASE, PAGE_SIZE);
	if (base == NULL) {
		pr_err("ioremap failed\n");
	}
	writel(LEVEL1_0, base);
	msleep(100);
	writel(LEVEL1_1, base);
	msleep(20);

	iounmap(base);
}

static void performance_opt(void)
{
	unsigned int chip_id;

	chip_id = get_chipid();
	switch (chip_id) {
	case CHIPID_XM72050500:
	case CHIPID_XM72050230:
	case CHIPID_XM72050200:
	case CHIPID_XM72010200:
	case CHIPID_XM72050210:
		opt_method0();
		fephy_private.compat_state = 1;
		break;
	case CHIPID_XM72050510:
	case CHIPID_XM72050530:
	case CHIPID_XM76050100:
	case CHIPID_XM72050300:
		opt_method1();
		fephy_private.compat_state = 1;
		break;
	default:
		fephy_private.compat_state = 0;
		return;
	}
}

static void compat_phy_init(struct femac_mdio_data *data)
{
	struct mii_bus *bus = data->bus;
	u32 phy_addr = data->phy_addr;
	u32 val;
	...
	g_mdio_data = data;
	val = fephy_expanded_read(bus, phy_addr, REG_FW_STATE);
	if (!val) {
		fephy_private.compat_state = 0;
		return;
	}
...
static int femac_mdio_probe(struct platform_device *pdev)
{
	...
	chip_id = get_chipid();
	if ((chip_id == CHIPID_XM72020330) || (chip_id == CHIPID_XM72020300) || (chip_id == CHIPID_XM72010300))
		return 0;

	fephy_private.fephy_init_state = 0;
	...
	fephy_private.fephy_init_state = 1;
	fephy_private.get_fw_state = fephy_get_firmware_state;
	fephy_private.mask_fw_state = fephy_mask_firmware_state;
	fephy_private.fephy_update = fephy_reset;

	__fephy_set_private_date(&fephy_private);
...
MODULE_DESCRIPTION("Xmedia Fast Ethernet MAC MDIO interface driver");
