...
void bsp_set_cpu(unsigned int cpu, bool enable)
{
	...
	np = of_find_compatible_node(NULL, NULL, "xmedia,xm72010300-clock");
	if (!np) {
		pr_err("failed to find xmedia clock node\n");
		return;
	}
...
CPU_METHOD_OF_DECLARE(xm72010300_smp, "xmedia,xm72010300-smp", &bsp_smp_ops);
#endif /* CONFIG_SMP */
