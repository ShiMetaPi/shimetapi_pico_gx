...
/* XMedia does not use interrupt endpoints */
...
#if defined(CONFIG_ARCH_XMFALCON) || defined(CONFIG_ARCH_XMORCA)
		/* XMedia does not have interrupt control endpoints */
#else
...
uvc_copy_descriptors(struct uvc_device *uvc, enum usb_device_speed speed)
{
	...
	if (opts->streaming_bulk) {
		bytes = uvc_iad.bLength + uvc_control_intf.bLength
			+ uvc_bulk_streaming_intf_alt0.bLength;
	} else {
		bytes = uvc_iad.bLength + uvc_control_intf.bLength
...
#if defined(CONFIG_ARCH_XMFALCON) || defined(CONFIG_ARCH_XMORCA)
	if (opts->streaming_bulk) {
		UVC_COPY_DESCRIPTOR(mem, dst, &uvc_bulk_streaming_intf_alt0);
	} else {
...
uvc_function_bind(struct usb_configuration *c, struct usb_function *f)
{
	...
	if (opts->streaming_bulk) {
		if (gadget_is_superspeed(c->cdev->gadget)) {
			ep = usb_ep_autoconfig_ss(cdev->gadget, &uvc_ss_bulk_streaming_ep,
					&uvc_ss_bulk_streaming_comp);
			uvc_ss_bulk_streaming_ep.wMaxPacketSize = cpu_to_le16(min(opts->streaming_maxpacket, 1024U));
			//uvcg_info(f, "Stream super-speed bulk endpoint maxpacket size is %d\n",
			//		uvc_ss_bulk_streaming_ep.wMaxPacketSize);
		} else if (gadget_is_dualspeed(cdev->gadget)) {
			ep = usb_ep_autoconfig(cdev->gadget, &uvc_hs_bulk_streaming_ep);
			uvc_hs_bulk_streaming_ep.wMaxPacketSize = cpu_to_le16(min(opts->streaming_maxpacket, 512U));
			//uvcg_info(f, "Stream high-speed bulk endpoint maxpacket size is %d\n",
			//		uvc_hs_bulk_streaming_ep.wMaxPacketSize);
		} else {
			ep = usb_ep_autoconfig(cdev->gadget, &uvc_fs_bulk_streaming_ep);
			uvc_fs_bulk_streaming_ep.wMaxPacketSize = cpu_to_le16(min(opts->streaming_maxpacket, 64U));
			//uvcg_info(f, "Stream full-speed bulk endpoint maxpacket size is %d\n",
			//		uvc_fs_bulk_streaming_ep.wMaxPacketSize);
		}
	} else {
		if (gadget_is_superspeed(c->cdev->gadget))
...
static struct usb_function_instance *uvc_alloc_inst(void)
{
	...
	int ret;
#if defined(CONFIG_ARCH_XMFALCON) || defined(CONFIG_ARCH_XMORCA)
	struct UVC_EXTENSION_UNIT_DESCRIPTOR(1, 4) *ed;
	static char extension_guid[16] = {'X', 'M', 'E', 'D', 'I', 'A', 'E', 'X',
                                      'T', 'E', 'N', 'S', 'I', 'O', 'N', '1'};
#endif
	...
	ed->bLength		= UVC_DT_EXTENSION_UNIT_SIZE(1, 4);
	...
	ed->bNumControls		= 32;
	ed->bControlSize		= 4;
	ed->bmControls[0]		= 0x01;
	ed->bmControls[1]		= 0x00;
	ed->bmControls[2]		= 0x00;
	ed->bmControls[3]		= 0x00;
	ed->iExtension		= 0;
#endif
