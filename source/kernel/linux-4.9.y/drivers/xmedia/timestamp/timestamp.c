...
#define CONVERT_TO_STAMP(value) (value)
#else
#define CONVERT_TO_STAMP(value) ((unsigned int)((u32)(value) * g_timestamp_ctx.timerpre / (g_timestamp_ctx.timer_clock / 1000U)))
#endif

u32 *reload_mem = NULL;
u32 *value_mem = NULL;
u32 *control_mem = NULL;
u32 *sysctl_mem =  NULL;
u32 *cnt_mem = NULL;
u32 *start_mem = NULL;
...
{
	reload_mem = ioremap(TIMER_REG_RELOAD, PAGE_SIZE);
	if (reload_mem == NULL) {
		printk(KERN_ERR "reload mem iommap failed\n");
		return -1;
	}
	value_mem = ioremap(TIMER_REG_VALUE, PAGE_SIZE);
	if (value_mem == NULL) {
		printk(KERN_ERR "value mem iommap failed\n");
		return -1;
	}
	control_mem = ioremap(TIMER_REG_CONTROL, PAGE_SIZE);
	if (control_mem == NULL) {
		printk(KERN_ERR "control mem iommap failed\n");
		return -1;
	}
	sysctl_mem =  ioremap(SYS_CTRL_REG_BASE + REG_SYS_CTRL_REG_BASE, PAGE_SIZE);
	if (sysctl_mem == NULL) {
		printk(KERN_ERR "sysctl mem iommap failed\n");
		return -1;
	}
	cnt_mem = ioremap(TIMESTAMP_CNT_ADDR, PAGE_SIZE);
	if (cnt_mem == NULL) {
		printk(KERN_ERR "cnt_mem mem iommap failed\n");
		return -1;
	}
	start_mem = ioremap(TIMESTAMP_ITEM_START_ADDR, PAGE_SIZE);
	if (start_mem == NULL) {
		printk(KERN_ERR "cnt_mem mem iommap failed\n");
		return -1;
	}
...
static int get_uboot_timestamp(void)
{
	...
	if ((cnt > 0) && (cnt < TIMESTAMP_MAX_CNT)) {
		...
		timestamp_uboot_item *uboot_items = (timestamp_uboot_item *)(start_mem);

		for (i = 0; i < cnt; i++) {
			if(uboot_items[i].type == UBOOT_TIMESTAMP) {
				timestamp_type time = CONVERT_TO_STAMP(uboot_items[i].stamp);
...
static int timestamp_init(unsigned int count)
{
	...
	spin_lock_init(&(g_timestamp_ctx.t_lock));
...
static void timestamp_deinit(void)
{
	if (g_timestamp_ctx.items != NULL) {
...
static void timestamp_get_time(timestamp_type *stamp)
{
	...
	*stamp = CONVERT_TO_STAMP(value);
	...
	tick = ~tick;
	clock = g_timestamp_ctx.timer_clock / 1000U;
	temp = (u64)tick * g_timestamp_ctx.timerpre;
	do_div(temp, clock);
	*stamp = temp;
#endif
}
...
static int timestamp_mark_api(const char *func, unsigned int line, unsigned int type)
{
	...
	timestamp_item *item = NULL;
	unsigned long flags;

#ifndef CONFIG_ARCH_XM720XXX
	spin_lock_irqsave(&(g_timestamp_ctx.t_lock), flags);
#endif
...
#endif
	if (g_timestamp_ctx.items == NULL) {
		spin_unlock_irqrestore(&(g_timestamp_ctx.t_lock), flags);
		return -2;
	}
	...
	item = g_timestamp_ctx.items;
	item[g_timestamp_ctx.item_cur_cnt].stamp = stamp;
	strncpy(item[g_timestamp_ctx.item_cur_cnt].func, func, sizeof(item[g_timestamp_ctx.item_cur_cnt].func) - 1);
...
int timestamp_print(unsigned int type)
{
	...
	for (i = 0; i < g_timestamp_ctx.item_cur_cnt; i++) {
...
static long timestamp_ioctl(struct file *file, unsigned int cmd, unsigned long pargs)
{
	...
	timestamp_item __user *args = NULL;
	...
	switch(cmd)
	{
		...
		case CMD_TIMESTAMP_PRINT:
			args = (void __user *)pargs;
			if (copy_to_user(args, &(g_timestamp_ctx.items[0]), sizeof(timestamp_item) * g_timestamp_ctx.item_cur_cnt))
				return -EFAULT;
			return 0;
...
static int stopwatch_init(void)
{
	reg_trigger = ioremap(MISC_CTRL62, PAGE_SIZE);
	if (reg_trigger == NULL) {
