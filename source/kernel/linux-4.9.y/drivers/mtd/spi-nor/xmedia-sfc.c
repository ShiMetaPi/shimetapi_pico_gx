...
/*****************************************************************************/
static int bsp_spi_nor_op_reg_with_dummy(struct spi_nor *nor,
				u8 opcode, int data_len, u8 optype,
				u32 addr, u8 addr_len,  u8 dummy_len)
{
	struct fmc_priv *priv = nor->priv;
	struct fmc_host *host = priv->host;
	u32 reg;
	...
	reg = FMC_DATA_NUM_CNT(data_len);
	fmc_writel(host, FMC_DATA_NUM, reg);


	reg = OP_CFG_FM_CS(priv->chipselect) | OP_CFG_OEN_EN;
	if (addr_len)
		reg |= OP_CFG_ADDR_NUM(addr_len);
	if (dummy_len)
		reg |= OP_CFG_DUMMY_NUM(dummy_len);
...
int bsp_spi_nor_read_reg_with_dummy(struct spi_nor *nor, u8 opcode, u8 *buf, int len, u32 addr, u8 addr_len, u8 dummy_len)
{
	...
	ret = bsp_spi_nor_op_reg_with_dummy(nor, opcode, len, FMC_OP_READ_DATA_EN, addr, addr_len, dummy_len);
	if (ret)
		return ret;
...
MODULE_DESCRIPTION("Xmedia SPI Nor Flash Controller Driver");
