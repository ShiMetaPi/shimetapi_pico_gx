...
static int lotus_fephy_v272_fix(struct phy_device *phy_dev)
{
	int count;
...
static int lotus_fephy_v115_fix(struct phy_device *phy_dev)
{
	int count;
...
{
	phy_register_fixup_for_uid(LOTUS_PHY_ID_V272,
							   LOTUS_PHY_MASK,
							   lotus_fephy_v272_fix);

	phy_register_fixup_for_uid(LOTUS_PHY_ID_V115,
							   LOTUS_PHY_MASK,
							   lotus_fephy_v115_fix);
}

void phy_unregister_fixups(void)
{
	phy_unregister_fixup_for_uid(LOTUS_PHY_ID_V272,
								 LOTUS_PHY_MASK);

	phy_unregister_fixup_for_uid(LOTUS_PHY_ID_V115,
								 LOTUS_PHY_MASK);
}
